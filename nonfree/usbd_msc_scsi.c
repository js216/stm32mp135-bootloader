...
uint8_t SCSI_ProcessCmd(USBD_HandleTypeDef *pdev, uint8_t lun, uint8_t *cmd)
{
   ...
   switch (cmd[0]) {
      ...
      case SCSI_REQUEST_SENSE: ret = SCSI_RequestSense(pdev, lun, cmd); break;

      case SCSI_INQUIRY: ret = SCSI_Inquiry(pdev, lun, cmd); break;
      ...
      case SCSI_MODE_SENSE6: ret = SCSI_ModeSense6(pdev, lun, cmd); break;

      case SCSI_MODE_SENSE10: ret = SCSI_ModeSense10(pdev, lun, cmd); break;
      ...
      case SCSI_READ10: ret = SCSI_Read10(pdev, lun, cmd); break;

      case SCSI_READ12: ret = SCSI_Read12(pdev, lun, cmd); break;

      case SCSI_WRITE10: ret = SCSI_Write10(pdev, lun, cmd); break;

      case SCSI_WRITE12: ret = SCSI_Write12(pdev, lun, cmd); break;

      case SCSI_VERIFY10: ret = SCSI_Verify10(pdev, lun, cmd); break;
                                  ...
                                  uint8_t *params)
{
   ...
   if (((USBD_StorageTypeDef *)pdev->pUserData[pdev->classId])->IsReady(lun) !=
       0) {
      SCSI_SenseCode(pdev, lun, NOT_READY, MEDIUM_NOT_PRESENT);
      hmsc->bot_state = USBD_BOT_NO_DATA;
                            ...
                            uint8_t *params)
{
   ...
   if ((params[1] & 0x01U) != 0U) /* Evpd is set */
   {
      ...
      {
         (void)SCSI_UpdateBotData(hmsc, MSC_Page00_Inquiry_Data,
                                  LENGTH_INQUIRY_PAGE00);
      } else if (params[2] ==
                 0x80U) /* Request for VPD page 0x80 Unit Serial Number */
      {
         (void)SCSI_UpdateBotData(hmsc, MSC_Page80_Inquiry_Data,
                                 ...
                                 uint8_t *params)
{
   ...
   if ((hmsc->scsi_sense_head != hmsc->scsi_sense_tail)) {
      ...
      hmsc->bot_data[12] =
          (uint8_t)hmsc->scsi_sense[hmsc->scsi_sense_head].w.b.ASC;
      hmsc->bot_data[13] =
          (uint8_t)hmsc->scsi_sense[hmsc->scsi_sense_head].w.b.ASCQ;
                                  ...
                                  uint8_t *params)
{
   ...
   if ((hmsc->scsi_medium_state == SCSI_MEDIUM_LOCKED) &&
       ((params[4] & 0x3U) == 2U)) {
