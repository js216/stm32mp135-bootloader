...
HAL_StatusTypeDef HAL_LTDC_Init(LTDC_HandleTypeDef *hltdc)
{
   ...
   if (hltdc->State == HAL_LTDC_STATE_RESET) {
      ...
      hltdc->LineEventCallback =
          HAL_LTDC_LineEventCallback; /* Legacy weak LineEventCallback    */
      hltdc->ReloadEventCallback =
          HAL_LTDC_ReloadEventCallback; /* Legacy weak ReloadEventCallback  */
      hltdc->ErrorCallback =
   ...
   hltdc->Instance->GCR |=
       (uint32_t)(hltdc->Init.HSPolarity | hltdc->Init.VSPolarity |
   ...
   hltdc->Instance->BCCR &=
       ~(LTDC_BCCR_BCBLUE | LTDC_BCCR_BCGREEN | LTDC_BCCR_BCRED);
   hltdc->Instance->BCCR |= (tmp1 | tmp | hltdc->Init.Backcolor.Blue);

   /* Configure the output to YCbCr 422: Enable, the CCIR hard-wired
    * coefficients, chrominance order  */
   hltdc->Instance->EDCR &=
       ~(LTDC_EDCR_OCYEN | LTDC_EDCR_OCYSEL | LTDC_EDCR_OCYCO);
   hltdc->Instance->EDCR |=
       (uint32_t)(hltdc->Init.ExtDisp.YCbCrEn | hltdc->Init.ExtDisp.YCbCrSel |
                          ...
                          pLTDC_CallbackTypeDef pCallback)
{
   ...
   if (hltdc->State == HAL_LTDC_STATE_READY) {
      switch (CallbackID) {
         ...
         case HAL_LTDC_ERROR_CB_ID: hltdc->ErrorCallback = pCallback; break;
         ...
         case HAL_LTDC_MSPINIT_CB_ID: hltdc->MspInitCallback = pCallback; break;
   ...
   if (hltdc->State == HAL_LTDC_STATE_READY) {
      switch (CallbackID) {
         ...
         case HAL_LTDC_SECURE_RELOAD_EVENT_CB_ID:
            ...
            hltdc->SecureReloadEventCallback =
                HAL_LTDC_Secure_ReloadEventCallback;
            break;
         ...
         case HAL_LTDC_MSPDEINIT_CB_ID:
            hltdc->MspDeInitCallback =
                HAL_LTDC_MspDeInit; /* Legcay weak MspDeInit Callback     */
            break;
...
void HAL_LTDC_IRQHandler(LTDC_HandleTypeDef *hltdc)
{
   ...
   if (((isrflags & LTDC_ISR_CRCIF) != 0U) &&
       ((itsources & LTDC_IER_CRCIE) != 0U)) {
    ...
    * ***************************************/
   if (((isrflags & LTDC_ISR_FUKIF) != 0U) &&
       ((itsources & LTDC_IER_FUKIE) != 0U)) {
      /* Disable the transfer Error interrupt */
    ...
    * ***************************************/
   if (((isrflags & LTDC_ISR_TERRIF) != 0U) &&
       ((itsources & LTDC_IER_TERRIE) != 0U)) {
      /* Disable the transfer Error interrupt */
    ...
    * ***************************************/
   if (((isrflags & LTDC_ISR_FUWIF) != 0U) &&
       ((itsources & LTDC_IER_FUWIE) != 0U)) {
    ...
    * ************************************************/
   if (((isrflags & LTDC_ISR_LIF) != 0U) &&
       ((itsources & LTDC_IER_LIE) != 0U)) {
    ...
    * ***************************************/
   if (((isrflags & LTDC_ISR_RRIF) != 0U) &&
       ((itsources & LTDC_IER_RRIE) != 0U)) {
...
__weak void HAL_LTDC_LineEventCallback(LTDC_HandleTypeDef *hltdc)
{
   ...
   /* NOTE : This function should not be modified, when the callback is needed,
             the HAL_LTDC_LineEventCallback could be implemented in the user
      file
    */
}
...
__weak void HAL_LTDC_ReloadEventCallback(LTDC_HandleTypeDef *hltdc)
{
   ...
   /* NOTE : This function should not be modified, when the callback is needed,
             the HAL_LTDC_ReloadEvenCallback could be implemented in the user
      file
    */
}
...
__weak void HAL_LTDC_Secure_LineEventCallback(LTDC_HandleTypeDef *hltdc)
{
   ...
   /* NOTE : This function should not be modified, when the callback is needed,
             the HAL_LTDC_SLineEventCallback could be implemented in the user
      file
    */
}
...
__weak void HAL_LTDC_Secure_ReloadEventCallback(LTDC_HandleTypeDef *hltdc)
{
   ...
   /* NOTE : This function should not be modified, when the callback is needed,
             the HAL_LTDC_Secure_ReloadEvenCallback could be implemented in the
      user file
    */
}
                                             ...
                                             uint32_t LayerIdx)
{
   ...
   LTDC_LAYER(hltdc, LayerIdx)->CKCR &=
                                      ...
                                      uint32_t LayerIdx)
{
   ...
   for (counter = 0U; (counter < CLUTSize); counter++) {
      if (hltdc->LayerCfg[LayerIdx].PixelFormat == LTDC_PIXEL_FORMAT_AL44) {
         tmp = (((counter + (16U * counter)) << 24U) |
                ((uint32_t)(*pcolorlut) & 0xFFU) |
                                       ...
                                       uint32_t GammaSize)
{
   ...
   for (i = 0U; (i < GammaSize); i++) {
      hltdc->Instance->GCCR = LTDC_GCCR_REN | pGamma[i].Address |
                              ((uint32_t)pGamma[i].Red << LTDC_GCCR_COMP_Pos);
      hltdc->Instance->GCCR = LTDC_GCCR_GEN | pGamma[i].Address |
                              ((uint32_t)pGamma[i].Green << LTDC_GCCR_COMP_Pos);
      hltdc->Instance->GCCR = LTDC_GCCR_BEN | pGamma[i].Address |
...
/**
 * @brief  Function used to reconfigure the pitch for specific cases where the
 * attached LayerIdx buffer have a width that islarger than the one intended to
 * be displayed on screen. Example of a buffer 800x480 attached to layer for
 * which we want to read and display on screen only a portion 320x240 taken in
 * the center of the buffer. The pitch in pixels will be in that case 800 pixels
 * and not 320 pixels as initially configured by previous call to
                           ...
                           LTDC_LayerCfgTypeDef *pLayerCfg, uint32_t LayerIdx)
{
   ...
   tmp = ((pLayerCfg->WindowX1 +
           ((hltdc->Instance->BPCR & LTDC_BPCR_AHBP) >> 16U))
          << 16U);
   LTDC_LAYER(hltdc, LayerIdx)->WHPCR &=
       ~(LTDC_LxWHPCR_WHSTPOS | LTDC_LxWHPCR_WHSPPOS);
   LTDC_LAYER(hltdc, LayerIdx)->WHPCR =
       ((pLayerCfg->WindowX0 +
   ...
   LTDC_LAYER(hltdc, LayerIdx)->WVPCR =
       ((pLayerCfg->WindowY0 + (hltdc->Instance->BPCR & LTDC_BPCR_AVBP) + 1U) |
        tmp);
   ...
   LTDC_LAYER(hltdc, LayerIdx)->DCCR &=
   ...
   switch (pLayerCfg->PixelFormat) {
      ...
      case LTDC_PIXEL_FORMAT_AL88:
         ...
         LTDC_LAYER(hltdc, LayerIdx)->FPF0R =
             (RLEN << 14U) + (RPOS << 9U) + (ALEN << 5U) + APOS;
         LTDC_LAYER(hltdc, LayerIdx)->FPF1R = (PSIZE << 18U) + (BLEN << 14U) +
                                              (BPOS << 9U) + (GLEN << 5U) +
                                              GPOS;
         break;
      default: break;
   ...
   switch (pLayerCfg->PixelFormat) {
      ...
      case LTDC_PIXEL_FORMAT_ABGR8888:
      case LTDC_PIXEL_FORMAT_RGBA8888:
      case LTDC_PIXEL_FORMAT_BGRA8888: stride = 4U; break;
      case LTDC_PIXEL_FORMAT_RGB888: stride = 3U; break;
      case LTDC_PIXEL_FORMAT_RGB565:
      case LTDC_PIXEL_FORMAT_BGR565:
      case LTDC_PIXEL_FORMAT_ARGB1555:
      case LTDC_PIXEL_FORMAT_ARGB4444:
      ...
      case LTDC_PIXEL_FORMAT_YUYV: stride = 2U; break;
      case LTDC_PIXEL_FORMAT_L8:
      case LTDC_PIXEL_FORMAT_AL44:
      default: stride = 1U; break;
   }
   ...
   LTDC_LAYER(hltdc, LayerIdx)->CFBLR =
       ((((0x10000U - (pLayerCfg->ImageWidth * stride))) << 16U) |
        (((pLayerCfg->WindowX1 - pLayerCfg->WindowX0) * stride) + 7U));

   /* YUV configurations */
   if ((LTDC_LAYER(hltdc, LayerIdx)->C1R &
        (LTDC_LxC1R_YIA | LTDC_LxC1R_YSPA | LTDC_LxC1R_YFPA)) != 0U) {
      LTDC_LAYER(hltdc, LayerIdx)->PCR &=
          ~(LTDC_LxPCR_VPDEN | LTDC_LxPCR_HPDEN | LTDC_LxPCR_YCEN |
            LTDC_LxPCR_YCM | LTDC_LxPCR_YF | LTDC_LxPCR_CBF | LTDC_LxPCR_YREN);
      switch (pLayerCfg->PixelFormat) {
         case LTDC_PIXEL_FORMAT_UYVY:
         ...
         case LTDC_PIXEL_FORMAT_YUYV:
            LTDC_LAYER(hltdc, LayerIdx)->PCR =
                LTDC_LxPCR_YCEN | LTDC_LxPCR_YF | LTDC_LxPCR_CBF;
            break;
         case LTDC_PIXEL_FORMAT_YVYU:
         ...
         case LTDC_PIXEL_FORMAT_NV12:
            LTDC_LAYER(hltdc, LayerIdx)->PCR =
                LTDC_LxPCR_YCEN | (0x1U << LTDC_LxPCR_YCM_Pos) | LTDC_LxPCR_CBF;
            break;
         case LTDC_PIXEL_FORMAT_NV21:
            LTDC_LAYER(hltdc, LayerIdx)->PCR =
                LTDC_LxPCR_YCEN | (0x1U << LTDC_LxPCR_YCM_Pos);
            break;
         case LTDC_PIXEL_FORMAT_YUV420:
            LTDC_LAYER(hltdc, LayerIdx)->PCR =
                LTDC_LxPCR_YCEN | (0x2U << LTDC_LxPCR_YCM_Pos) | LTDC_LxPCR_CBF;
            break;
         case LTDC_PIXEL_FORMAT_YVU420:
            LTDC_LAYER(hltdc, LayerIdx)->PCR =
                LTDC_LxPCR_YCEN | (0x2U << LTDC_LxPCR_YCM_Pos);
            break;
         default: break;
      }
   }
   ...
   LTDC_LAYER(hltdc, LayerIdx)->BFCR &=
       ~(LTDC_LxBFCR_BOR | LTDC_LxBFCR_BF2 | LTDC_LxBFCR_BF1);
   tmp = ((uint32_t)(pLayerCfg->BlendingOrder) << 16U);
   LTDC_LAYER(hltdc, LayerIdx)->BFCR =
       (pLayerCfg->BlendingFactor1 | pLayerCfg->BlendingFactor2 | tmp);
   ...
   if ((LTDC_LAYER(hltdc, LayerIdx)->C1R &
      ...
      LTDC_LAYER(hltdc, LayerIdx)->CYR0R &=
          ~(LTDC_LxCYR0R_CB2B | LTDC_LxCYR0R_CR2R);
      LTDC_LAYER(hltdc, LayerIdx)->CYR0R = 0x02040199U;

      /* Configure the conversion YCbCr RGB */
      LTDC_LAYER(hltdc, LayerIdx)->CYR1R &=
          ~(LTDC_LxCYR1R_CR2G | LTDC_LxCYR1R_CB2G);
   ...
   if ((!pLayerCfg->HorMirrorEn) && (!pLayerCfg->VertMirrorEn)) {
      ...
      switch (pLayerCfg->PixelFormat) {
         ...
         case LTDC_PIXEL_FORMAT_NV21:
            ...
            LTDC_LAYER(hltdc, LayerIdx)->AFBA0R =
                pLayerCfg->AuxiliaryFB.StartAddressBuffer0;

            /* Configure the auxiliary frame buffer address 1 */
            LTDC_LAYER(hltdc, LayerIdx)->AFBA1R &= ~(LTDC_LxAFBA1R_AFBADD1);
            LTDC_LAYER(hltdc, LayerIdx)->AFBA1R =
                pLayerCfg->AuxiliaryFB.StartAddressBuffer1;

            /* Configure the buffer length */
            LTDC_LAYER(hltdc, LayerIdx)->AFBLR &=
                ~(LTDC_LxAFBLR_AFBLL | LTDC_LxAFBLR_AFBP);
            LTDC_LAYER(hltdc, LayerIdx)->AFBLR =
                (pLayerCfg->ImageWidth << 16U) |
                (pLayerCfg->WindowX1 - pLayerCfg->WindowX0 + 7U);
            /* Configure the frame buffer line number */
            LTDC_LAYER(hltdc, LayerIdx)->AFBLNR &= ~(LTDC_LxAFBLNR_AFBLNBR);
            LTDC_LAYER(hltdc, LayerIdx)->AFBLNR =
                (pLayerCfg->ImageHeight) >> 1U;
            break;
         case LTDC_PIXEL_FORMAT_YUV420:
         case LTDC_PIXEL_FORMAT_YVU420:
            /* Configure the auxiliary frame buffer address 0 */
      ...
      LTDC_LAYER(hltdc, LayerIdx)->CFBLR &=
          ~(LTDC_LxCFBLR_CFBLL | LTDC_LxCFBLR_CFBP);
      LTDC_LAYER(hltdc, LayerIdx)->CFBLR =
          (((pLayerCfg->ImageWidth * stride) << 16U) |
           (((pLayerCfg->WindowX1 - pLayerCfg->WindowX0) * stride) + 7U));
   ...
   if ((pLayerCfg->HorMirrorEn) && (!pLayerCfg->VertMirrorEn)) {
      ...
      LTDC_LAYER(hltdc, LayerIdx)->CFBAR &= ~(LTDC_LxCFBAR_CFBADD);
      LTDC_LAYER(hltdc, LayerIdx)->CFBAR =
      ...
      LTDC_LAYER(hltdc, LayerIdx)->CR =
          (uint32_t)LTDC_LxCR_HMEN | LTDC_LxCR_LEN;
   }
      ...
      switch (pLayerCfg->PixelFormat) {
         ...
         case LTDC_PIXEL_FORMAT_YVU420:
            ...
            LTDC_LAYER(hltdc, LayerIdx)->AFBLR &=
                ~(LTDC_LxAFBLR_AFBLL | LTDC_LxAFBLR_AFBP);
            LTDC_LAYER(hltdc, LayerIdx)->AFBLR =
                (((0x10000U - (pLayerCfg->ImageWidth >> 1U)) * stride) << 16U) |
                ((((pLayerCfg->WindowX1 - pLayerCfg->WindowX0) >> 1U) *
                  stride) +
                 7U);
   ...
   if ((pLayerCfg->HorMirrorEn) && (pLayerCfg->VertMirrorEn)) {
      ...
      LTDC_LAYER(hltdc, LayerIdx)->CFBAR =
          pLayerCfg->FBStartAdress +
          (stride * (pLayerCfg->WindowX1 - pLayerCfg->WindowX0) *
           (pLayerCfg->WindowY1 - pLayerCfg->WindowY0)) -
          1U;
