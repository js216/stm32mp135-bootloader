...
typedef struct {
   ...
   uint32_t
       DMADisableonRxError; /*!< Specifies whether the DMA is disabled in case
                               of reception error. This parameter can be a value
                               of @ref UART_DMA_Disable_on_Rx_Error. */
...
typedef enum {
   ...
   UART_CLOCKSOURCE_PCLK5 =
       0x02U, /*!< PCLK5 clock source  (only used by UART1) */
   UART_CLOCKSOURCE_PLL3Q =
       0x04U, /*!< PLL3Q clock source  (only used by UART1) */
 ...
 *         The maximum Baud Rate is derived from the maximum clock on MP1 (i.e.
 * 100 MHz) divided by the smallest oversampling used on the USART (i.e. 8)
...
#define IS_UART_PARITY(__PARITY__)                                             \
   (((__PARITY__) == UART_PARITY_NONE) ||                                      \
    ((__PARITY__) == UART_PARITY_EVEN) || ((__PARITY__) == UART_PARITY_ODD))
