/**
  ...
  ******************************************************************************
  ...
  interrupt has occurred or not.
     ...
     [..]
      (@) You can refer to the DMA HAL driver header file for more useful
  macros.
...
@verbatim
 ...
 ===============================================================================
    ...
    circular/normal mode selection, memory-to-memory mode selection and Stream
priority value.
    [..]
    The HAL_DMA_Init() function follows the DMA configuration procedures as
described in reference manual. The HAL_DMA_DeInit function allows to
...
HAL_StatusTypeDef HAL_DMA_Init(DMA_HandleTypeDef *hdma)
{
   ...
   if (IS_DMA_INSTANCE(hdma) != 0U) /* DMA1 or DMA2 instance */
   {
      ...
      /* Check the memory burst, peripheral burst and FIFO threshold parameters
         only when FIFO mode is enabled */
      ...
      /* Clear CHSEL, MBURST, PBURST, PL, MSIZE, PSIZE, MINC, PINC, CIRC, DIR,
       * CT and DBM bits */
      registerValue &= ((uint32_t)~(
          DMA_SxCR_MBURST | DMA_SxCR_PBURST | DMA_SxCR_PL | DMA_SxCR_MSIZE |
      ...
      if (hdma->Init.FIFOMode == DMA_FIFOMODE_ENABLE) {
         ...
         /* Check compatibility between FIFO threshold level and size of the
          * memory burst */
                        ...
                        uint32_t Timeout)
{
   ...
   while (((*isr_reg) & cpltlevel_mask) == 0U) {
      if (IS_DMA_INSTANCE(hdma) != 0U) /* DMA1 or DMA2 instance */
      {
         ...
         if (((*isr_reg) &
              (DMA_FLAG_DMEIF0_4 << (hdma->StreamIndex & 0x1FU))) != 0U) {
            /* Update error code */
   ...
   if (Timeout != HAL_MAX_DELAY) {
      if (((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) {
         ...
         /* No need to check return value: as in this case we will return
            HAL_ERROR with HAL_DMA_ERROR_TIMEOUT error code  */
      ...
      if ((tmpisr_dma & (DMA_FLAG_HTIF0_4 << (hdma->StreamIndex & 0x1FU))) !=
         ...
         if (__HAL_DMA_GET_IT_SOURCE(hdma, DMA_IT_HT) != 0U) {
            ...
            if (((((DMA_Stream_TypeDef *)hdma->Instance)->CR) &
               ...
               if ((((DMA_Stream_TypeDef *)hdma->Instance)->CR & DMA_SxCR_CT) ==
                   0U) {
                  if (hdma->XferHalfCpltCallback != NULL) {
                           ...
                           HAL_DMA_CallbackIDTypeDef CallbackID)
{
   ...
   if (HAL_DMA_STATE_READY == hdma->State) {
      switch (CallbackID) {
         case HAL_DMA_XFER_CPLT_CB_ID: hdma->XferCpltCallback = NULL; break;

         case HAL_DMA_XFER_HALFCPLT_CB_ID:
            hdma->XferHalfCpltCallback = NULL;
            break;

         case HAL_DMA_XFER_M1CPLT_CB_ID: hdma->XferM1CpltCallback = NULL; break;

         case HAL_DMA_XFER_M1HALFCPLT_CB_ID:
            hdma->XferM1HalfCpltCallback = NULL;
            break;

         case HAL_DMA_XFER_ERROR_CB_ID: hdma->XferErrorCallback = NULL; break;

         case HAL_DMA_XFER_ABORT_CB_ID: hdma->XferAbortCallback = NULL; break;
...
static HAL_StatusTypeDef DMA_CheckFifoParam(const DMA_HandleTypeDef *hdma)
{
   ...
   if (hdma->Init.MemDataAlignment == DMA_MDATAALIGN_BYTE) {
      switch (hdma->Init.FIFOThreshold) {
         ...
         case DMA_FIFO_THRESHOLD_3QUARTERSFULL:
            ...
            if ((hdma->Init.MemBurst & DMA_SxCR_MBURST_1) ==
                DMA_SxCR_MBURST_1) {
               status = HAL_ERROR;
            }
            break;
         ...
         case DMA_FIFO_THRESHOLD_FULL: break;
   ...
   /* Memory Data size equal to Half-Word */
      ...
      switch (hdma->Init.FIFOThreshold) {
         case DMA_FIFO_THRESHOLD_1QUARTERFULL:
         case DMA_FIFO_THRESHOLD_3QUARTERSFULL: status = HAL_ERROR; break;
