  ...
  ******************************************************************************
  ...
  ==============================================================================
    ...
    [..]
      ...
      (+) Configure the clock kernel source(s) for peripherals which clocks are
  not derived from Bus clocks
...
#define HSE_MASK                                                               \
   (RCC_OCENSETR_HSEON | RCC_OCENSETR_HSEBYP | RCC_OCENSETR_DIGBYP)
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
 ...
 */
HAL_StatusTypeDef
RCC_AXISSConfig(const RCC_AXISSInitTypeDef *RCC_AXISSInitStruct);
...
internal/external oscillators (HSE, HSI, LSE,CSI, LSI) PLL, HSECSS, LSECSS and
MCO and the System busses clocks ( MPUSS,AXISS, MLAHBSS, AHB[1:4], AHB[5:6],
APB1, APB2, APB3, APB4 and APB5).
...
source.
         ...
         (#) PLL , The RCC features four independent PLLs (clocked by HSI , HSE
or CSI), featuring three different output clocks and able  to work either in
integer, Fractional or Spread Sprectum mode.
           (++) PLL1, which is used to provide clock to the A7 sub-system
           (++) PLL2, which is used to provide clock to the AXI sub-system, DDR
and GPU
           (++) PLL3, which is used to provide clock to the MLAHB sub-system and
to generate the kernel clock for peripherals.
           (++) PLL4, which is used to generate the kernel clock for
peripherals.


         (#) HSECSS (HSE Clock security system), once enabled and if a HSE clock
failure occurs HSECSS will generate a system reset, and protect sensitive data
of the BKPSRAM

         (#) LSECSS (LSE Clock security system), once enabled and if a LSE clock
failure occurs
             (++) The clock provided to the RTC is disabled immediately by the
hardware
             (++) A failure event is generated (rcc_lsecss_fail). This event is
connected to the TAMP block, allowing to wake up from Standby, and allowing the
protection of backup registers and BKPSRAM
             (++) An interrupt flag (LSECSSF) is activated in order to generate
an interrupt to the MLAHB or MPU if needed.

         (#) MCO1 (micro controller clock output), used to output HSI, HSE,
CSI,LSI or LSE clock (through a configurable pre-scaler).

         (#) MCO2 (micro controller clock output), used to output MPUSS, AXISS,
MLAHBSS, PLL4(PLL4_P), HSE, or HSI clock (through a configurable pre-scaler).


    [..] Sub-system clock configuration

         (#) The MPUDIV divider located into RCC MPU Clock Divider Register
(RCC_MPCKDIVR) can be used to adjust dynamically the clock for the MPU.

         (#) The AXIDIV divider located into RCC AXI Clock Divider Register
(RCC_AXIDIVR) can be used to adjust dynamically the clock for the AXI matrix.
The value of this divider also impacts the clock frequency of AHB5, AHB6, APB4
and APB5.

         (#) The APB4DIV divider located into RCC APB4 Clock Divider Register
(RCC_APB4DIVR) can be used to adjust dynamically the clock for the APB4 bridge.

         (#) In the same way, the APB5DIV divider located into RCC APB5 Clock
Divider Register (RCC_APB5DIVR), can be used to adjust dynamically the clock for
the APB5 bridge.

         (#) In the MLAHB clock tree, the MLAHBDIV divider located into RCC
MLAHB Clock Divider Register (RCC_MLAHBDIVR), can be used to adjust dynamically
the clock for the MLAHB. The value of this divider also impacts the clock
frequency of AHB bridges, APB1, APB2 and APB3.

         (#) The APB1DIV divider located into RCC APB1 Clock Divider Register
(RCC_APB1DIVR), can be used to adjust dynamically the clock for the APB1 bridge.
Similar implementation is available for APB2 and APB3.
 ...
 *            - MCO1 and MCO2 OFF
 *            - All interrupts disabled (except Wake-up from CSTOP Interrupt
 * Enable)
...
HAL_StatusTypeDef HAL_RCC_DeInit(void)
{
   ...
   SET_BIT(RCC->MP_CIFR,
           (RCC_MP_CIFR_LSIRDYF | RCC_MP_CIFR_LSERDYF | RCC_MP_CIFR_HSIRDYF |
 ...
 *         parameter State
 * @note   Beware HSE oscillator is not used as clock before using this
 * function. If this is the case, you have to select another source clock for
 * item using HSE then change the HSE state (Eg.: disable it).
 ...
 * @note   Beware LSE oscillator is not used as clock before using this
 * function. If this is the case, you have to select another source clock for
 * item using LSE then change the LSE state (Eg.: disable it).
...
/**
 * @brief  Initializes the MPU,AXI, AHB and APB busses clocks according to the
 * specified parameters in the RCC_ClkInitStruct.
 * @param  RCC_ClkInitStruct: pointer to an RCC_OscInitTypeDef structure that
 *         contains the configuration information for the RCC peripheral.
 *
 * @note   The SystemCoreClock CCSIS variable is used to store System Clock
...
HAL_RCC_ClockConfig(const RCC_ClkInitTypeDef *RCC_ClkInitStruct)
{
   ...
   if ((RCC_ClkInitStruct->ClockType & RCC_CLOCKTYPE_MPU) ==
       RCC_CLOCKTYPE_MPU) {
   ...
   if ((RCC_ClkInitStruct->ClockType & RCC_CLOCKTYPE_PCLK6) ==
       RCC_CLOCKTYPE_PCLK6) {
 ...
 * @note   BSP_MCO_Init shall be called before to configure output pins
 ...
 *          This parameter can be one of the following values:
 *            @arg RCC_MCODIV_1 up to RCC_MCODIV_16  : divider applied to MCOx
 * clock
 * @retval None
 */
...
/**
  ...
  *         - The RCC generates a system reset (nreset). The flag HCSSRSTF is
  set in order to
  *           allow the application to identify the reset root cause.
  *         - A failure event is generated (rcc_hsecss_fail). This event is
  connected to the TAMP
  *           block, allowing the protection of backup registers and BKPSRAM.
  *
  * @note   HSECSSON can be activated even when the HSEON is set to 0. The CSS
  on HSE will be
  *         enabled by the hardware when the HSE is enabled and ready, and
  HSECSSON is set to 1.
  *
  * @note   The HSECSS is disabled when the HSE is disabled (i.e. when the
  system is STOP or
  *         STANDBY).
  *
  * @note   It is not possible to clear the bit HSECSSON by software.
  *
  * @note   The bit HSECSSON will be cleared by the hardware when a system reset
  occurs or when the system enters in STANDBY mode
  * @retval None
  */
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef *RCC_OscInitStruct)
{
   ...
   RCC_OscInitStruct->OscillatorType =
       ...
       RCC_OSCILLATORTYPE_CSI | RCC_OSCILLATORTYPE_LSE | RCC_OSCILLATORTYPE_LSI;
   ...
   RCC_OscInitStruct->CSICalibrationValue =
       (uint32_t)((RCC->CSICFGR & RCC_CSICFGR_CSITRIM) >>
   ...
   RCC_OscInitStruct->HSICalibrationValue =
       (uint32_t)((RCC->HSICFGR & RCC_HSICFGR_HSITRIM) >>
   ...
   RCC_OscInitStruct->PLL.PLLM =
       (uint32_t)((RCC->PLL1CFGR1 & RCC_PLL1CFGR1_DIVM1) >>
                  RCC_PLL1CFGR1_DIVM1_Pos) +
       1U;
   RCC_OscInitStruct->PLL.PLLN =
       (uint32_t)((RCC->PLL1CFGR1 & RCC_PLL1CFGR1_DIVN) >>
                  RCC_PLL1CFGR1_DIVN_Pos) +
       1U;
   RCC_OscInitStruct->PLL.PLLR =
       (uint32_t)((RCC->PLL1CFGR2 & RCC_PLL1CFGR2_DIVR) >>
                  RCC_PLL1CFGR2_DIVR_Pos) +
       1U;
   RCC_OscInitStruct->PLL.PLLP =
       (uint32_t)((RCC->PLL1CFGR2 & RCC_PLL1CFGR2_DIVP) >>
                  RCC_PLL1CFGR2_DIVP_Pos) +
       1U;
   RCC_OscInitStruct->PLL.PLLQ =
       (uint32_t)((RCC->PLL1CFGR2 & RCC_PLL1CFGR2_DIVQ) >>
                  RCC_PLL1CFGR2_DIVQ_Pos) +
       1U;
   RCC_OscInitStruct->PLL.PLLFRACV =
       (uint32_t)((RCC->PLL1FRACR & RCC_PLL1FRACR_FRACV) >>
   ...
   RCC_OscInitStruct->PLL.MOD_PER =
       (uint32_t)(RCC->PLL1CSGR & RCC_PLL1CSGR_MOD_PER);
   RCC_OscInitStruct->PLL.TPDFN_DIS =
       (uint32_t)(RCC->PLL1CSGR & RCC_PLL1CSGR_TPDFN_DIS);
   RCC_OscInitStruct->PLL.RPDFN_DIS =
       (uint32_t)(RCC->PLL1CSGR & RCC_PLL1CSGR_RPDFN_DIS);
   RCC_OscInitStruct->PLL.SSCG_MODE =
       (uint32_t)(RCC->PLL1CSGR & RCC_PLL1CSGR_SSCG_MODE);
   RCC_OscInitStruct->PLL.INC_STEP =
       (uint32_t)((RCC->PLL1CSGR & RCC_PLL1CSGR_INC_STEP) >>
                  RCC_PLL1CSGR_INC_STEP_Pos);
   ...
   RCC_OscInitStruct->PLL2.PLLSource =
       (uint32_t)(RCC->RCK12SELR & RCC_RCK12SELR_PLL12SRC);
   RCC_OscInitStruct->PLL2.PLLM =
       (uint32_t)((RCC->PLL2CFGR1 & RCC_PLL2CFGR1_DIVM2) >>
                  RCC_PLL2CFGR1_DIVM2_Pos) +
       1U;
   RCC_OscInitStruct->PLL2.PLLN =
       (uint32_t)((RCC->PLL2CFGR1 & RCC_PLL2CFGR1_DIVN) >>
                  RCC_PLL2CFGR1_DIVN_Pos) +
       1U;
   RCC_OscInitStruct->PLL2.PLLR =
       (uint32_t)((RCC->PLL2CFGR2 & RCC_PLL2CFGR2_DIVR) >>
                  RCC_PLL2CFGR2_DIVR_Pos) +
       1U;
   RCC_OscInitStruct->PLL2.PLLP =
       (uint32_t)((RCC->PLL2CFGR2 & RCC_PLL2CFGR2_DIVP) >>
                  RCC_PLL2CFGR2_DIVP_Pos) +
       1U;
   RCC_OscInitStruct->PLL2.PLLQ =
       (uint32_t)((RCC->PLL2CFGR2 & RCC_PLL2CFGR2_DIVQ) >>
                  RCC_PLL2CFGR2_DIVQ_Pos) +
       1U;
   RCC_OscInitStruct->PLL2.PLLFRACV =
       (uint32_t)((RCC->PLL2FRACR & RCC_PLL2FRACR_FRACV) >>
   ...
   RCC_OscInitStruct->PLL2.MOD_PER =
       (uint32_t)(RCC->PLL2CSGR & RCC_PLL2CSGR_MOD_PER);
   RCC_OscInitStruct->PLL2.TPDFN_DIS =
       (uint32_t)(RCC->PLL2CSGR & RCC_PLL2CSGR_TPDFN_DIS);
   RCC_OscInitStruct->PLL2.RPDFN_DIS =
       (uint32_t)(RCC->PLL2CSGR & RCC_PLL2CSGR_RPDFN_DIS);
   RCC_OscInitStruct->PLL2.SSCG_MODE =
       (uint32_t)(RCC->PLL2CSGR & RCC_PLL2CSGR_SSCG_MODE);
   RCC_OscInitStruct->PLL2.INC_STEP =
       (uint32_t)((RCC->PLL2CSGR & RCC_PLL2CSGR_INC_STEP) >>
                  RCC_PLL2CSGR_INC_STEP_Pos);
   ...
   RCC_OscInitStruct->PLL3.PLLSource =
       (uint32_t)(RCC->RCK3SELR & RCC_RCK3SELR_PLL3SRC);
   RCC_OscInitStruct->PLL3.PLLM =
       (uint32_t)((RCC->PLL3CFGR1 & RCC_PLL3CFGR1_DIVM3) >>
                  RCC_PLL3CFGR1_DIVM3_Pos) +
       1U;
   RCC_OscInitStruct->PLL3.PLLN =
       (uint32_t)((RCC->PLL3CFGR1 & RCC_PLL3CFGR1_DIVN) >>
                  RCC_PLL3CFGR1_DIVN_Pos) +
       1U;
   RCC_OscInitStruct->PLL3.PLLR =
       (uint32_t)((RCC->PLL3CFGR2 & RCC_PLL3CFGR2_DIVR) >>
                  RCC_PLL3CFGR2_DIVR_Pos) +
       1U;
   RCC_OscInitStruct->PLL3.PLLP =
       (uint32_t)((RCC->PLL3CFGR2 & RCC_PLL3CFGR2_DIVP) >>
                  RCC_PLL3CFGR2_DIVP_Pos) +
       1U;
   RCC_OscInitStruct->PLL3.PLLQ =
       (uint32_t)((RCC->PLL3CFGR2 & RCC_PLL3CFGR2_DIVQ) >>
                  RCC_PLL3CFGR2_DIVQ_Pos) +
       1U;
   RCC_OscInitStruct->PLL3.PLLRGE =
       (uint32_t)(RCC->PLL3CFGR1 & RCC_PLL3CFGR1_IFRGE);
   RCC_OscInitStruct->PLL3.PLLFRACV =
       (uint32_t)((RCC->PLL3FRACR & RCC_PLL3FRACR_FRACV) >>
   ...
   RCC_OscInitStruct->PLL3.MOD_PER =
       (uint32_t)(RCC->PLL3CSGR & RCC_PLL3CSGR_MOD_PER);
   RCC_OscInitStruct->PLL3.TPDFN_DIS =
       (uint32_t)(RCC->PLL3CSGR & RCC_PLL3CSGR_TPDFN_DIS);
   RCC_OscInitStruct->PLL3.RPDFN_DIS =
       (uint32_t)(RCC->PLL3CSGR & RCC_PLL3CSGR_RPDFN_DIS);
   RCC_OscInitStruct->PLL3.SSCG_MODE =
       (uint32_t)(RCC->PLL3CSGR & RCC_PLL3CSGR_SSCG_MODE);
   RCC_OscInitStruct->PLL3.INC_STEP =
       (uint32_t)((RCC->PLL3CSGR & RCC_PLL3CSGR_INC_STEP) >>
                  RCC_PLL3CSGR_INC_STEP_Pos);
   ...
   RCC_OscInitStruct->PLL4.PLLSource =
       (uint32_t)(RCC->RCK4SELR & RCC_RCK4SELR_PLL4SRC);
   RCC_OscInitStruct->PLL4.PLLM =
       (uint32_t)((RCC->PLL4CFGR1 & RCC_PLL4CFGR1_DIVM4) >>
                  RCC_PLL4CFGR1_DIVM4_Pos) +
       1U;
   RCC_OscInitStruct->PLL4.PLLN =
       (uint32_t)((RCC->PLL4CFGR1 & RCC_PLL4CFGR1_DIVN) >>
                  RCC_PLL4CFGR1_DIVN_Pos) +
       1U;
   RCC_OscInitStruct->PLL4.PLLR =
       (uint32_t)((RCC->PLL4CFGR2 & RCC_PLL4CFGR2_DIVR) >>
                  RCC_PLL4CFGR2_DIVR_Pos) +
       1U;
   RCC_OscInitStruct->PLL4.PLLP =
       (uint32_t)((RCC->PLL4CFGR2 & RCC_PLL4CFGR2_DIVP) >>
                  RCC_PLL4CFGR2_DIVP_Pos) +
       1U;
   RCC_OscInitStruct->PLL4.PLLQ =
       (uint32_t)((RCC->PLL4CFGR2 & RCC_PLL4CFGR2_DIVQ) >>
                  RCC_PLL4CFGR2_DIVQ_Pos) +
       1U;
   RCC_OscInitStruct->PLL4.PLLRGE =
       (uint32_t)(RCC->PLL4CFGR1 & RCC_PLL4CFGR1_IFRGE);
   RCC_OscInitStruct->PLL4.PLLFRACV =
       (uint32_t)((RCC->PLL4FRACR & RCC_PLL4FRACR_FRACV) >>
   ...
   RCC_OscInitStruct->PLL4.MOD_PER =
       (uint32_t)(RCC->PLL4CSGR & RCC_PLL4CSGR_MOD_PER);
   RCC_OscInitStruct->PLL4.TPDFN_DIS =
       (uint32_t)(RCC->PLL4CSGR & RCC_PLL4CSGR_TPDFN_DIS);
   RCC_OscInitStruct->PLL4.RPDFN_DIS =
       (uint32_t)(RCC->PLL4CSGR & RCC_PLL4CSGR_RPDFN_DIS);
   RCC_OscInitStruct->PLL4.SSCG_MODE =
       (uint32_t)(RCC->PLL4CSGR & RCC_PLL4CSGR_SSCG_MODE);
   RCC_OscInitStruct->PLL4.INC_STEP =
       (uint32_t)((RCC->PLL4CSGR & RCC_PLL4CSGR_INC_STEP) >>
                  RCC_PLL4CSGR_INC_STEP_Pos);
}
                            ...
                            const uint32_t *pFLatency)
{
   ...
   RCC_ClkInitStruct->ClockType =
       ...
       RCC_CLOCKTYPE_PCLK2 | RCC_CLOCKTYPE_PCLK3 | RCC_CLOCKTYPE_PCLK6;
...
/**
 * @brief  Returns the PLL1 clock frequencies :PLL1_P_Frequency,PLL1_R_Frequency
 * and PLL1_Q_Frequency
 * @note   The PLL1 clock frequencies computed by this function is not the real
 *         frequency in the chip. It is calculated based on the predefined
 *         constant and the selected clock source:
 * @note     The function returns values based on HSE_VALUE or HSI_VALUE Value
...
__weak void HAL_RCC_GetPLL1ClockFreq(PLL1_ClocksTypeDef *PLL1_Clocks)
{
   ...
   pll1m =
       ((RCC->PLL1CFGR1 & RCC_PLL1CFGR1_DIVM1) >> RCC_PLL1CFGR1_DIVM1_Pos) + 1U;
   pll1fracen =
       (RCC->PLL1FRACR & RCC_PLL1FRACR_FRACLE) >> RCC_PLL1FRACR_FRACLE_Pos;
   fracn1us = (pll1fracen * ((RCC->PLL1FRACR & RCC_PLL1FRACR_FRACV) >>
                             RCC_PLL1FRACR_FRACV_Pos));
   fracn1   = (float32_t)fracn1us;

   pll1vco_1 = (RCC->PLL1CFGR1 & RCC_PLL1CFGR1_DIVN) + 1U;
   pll1vco   = (float32_t)pll1vco_1 +
             (fracn1 / (float32_t)0x2000); /* Intermediary value */
   ...
   switch (pllsource) {
      ...
      default: pll1vco = (float32_t)(0); break;
   }

   P_Div =
       ((RCC->PLL1CFGR2 & RCC_PLL1CFGR2_DIVP) >> RCC_PLL1CFGR2_DIVP_Pos) + 1U;
   P_Freq                        = pll1vco / (float32_t)P_Div;
   PLL1_Clocks->PLL1_P_Frequency = (uint32_t)P_Freq;

   Q_Div =
       ((RCC->PLL1CFGR2 & RCC_PLL1CFGR2_DIVQ) >> RCC_PLL1CFGR2_DIVQ_Pos) + 1U;
   Q_Freq                        = pll1vco / (float32_t)Q_Div;
   PLL1_Clocks->PLL1_Q_Frequency = (uint32_t)Q_Freq;

   R_Div =
       ((RCC->PLL1CFGR2 & RCC_PLL1CFGR2_DIVR) >> RCC_PLL1CFGR2_DIVR_Pos) + 1U;
   R_Freq                        = pll1vco / (float32_t)R_Div;
   PLL1_Clocks->PLL1_R_Frequency = (uint32_t)R_Freq;
}

/**
 * @brief  Returns the PLL2 clock frequencies :PLL2_P_Frequency,PLL2_R_Frequency
 * and PLL2_Q_Frequency
...
__weak void HAL_RCC_GetPLL2ClockFreq(PLL2_ClocksTypeDef *PLL2_Clocks)
{
   ...
   pllsource = __HAL_RCC_GET_PLL12_SOURCE();
   pll2m =
       ((RCC->PLL2CFGR1 & RCC_PLL2CFGR1_DIVM2) >> RCC_PLL2CFGR1_DIVM2_Pos) + 1U;
   pll2fracen =
       (RCC->PLL2FRACR & RCC_PLL2FRACR_FRACLE) >> RCC_PLL2FRACR_FRACLE_Pos;

   fracn1us = (pll2fracen * ((RCC->PLL2FRACR & RCC_PLL2FRACR_FRACV) >>
                             RCC_PLL2FRACR_FRACV_Pos));
   fracn1   = (float32_t)fracn1us;
   ...
   switch (pllsource) {
      ...
      default: pll2vco = (float32_t)(0); break;
   }

   P_Div =
       ((RCC->PLL2CFGR2 & RCC_PLL2CFGR2_DIVP) >> RCC_PLL2CFGR2_DIVP_Pos) + 1U;
   P_Freq                        = pll2vco / (float32_t)P_Div;
   PLL2_Clocks->PLL2_P_Frequency = (uint32_t)P_Freq;

   Q_Div =
       ((RCC->PLL2CFGR2 & RCC_PLL2CFGR2_DIVQ) >> RCC_PLL2CFGR2_DIVQ_Pos) + 1U;
   Q_Freq                        = pll2vco / (float32_t)Q_Div;
   PLL2_Clocks->PLL2_Q_Frequency = (uint32_t)Q_Freq;

   R_Div =
       ((RCC->PLL2CFGR2 & RCC_PLL2CFGR2_DIVR) >> RCC_PLL2CFGR2_DIVR_Pos) + 1U;
   R_Freq                        = pll2vco / (float32_t)R_Div;
   PLL2_Clocks->PLL2_R_Frequency = (uint32_t)R_Freq;
}

/**
 * @brief  Returns the PLL3 clock frequencies :PLL3_P_Frequency,PLL3_R_Frequency
 * and PLL3_Q_Frequency
...
__weak void HAL_RCC_GetPLL3ClockFreq(PLL3_ClocksTypeDef *PLL3_Clocks)
{
   ...
   pll3m =
       ((RCC->PLL3CFGR1 & RCC_PLL3CFGR1_DIVM3) >> RCC_PLL3CFGR1_DIVM3_Pos) + 1U;
   pll3fracen =
       (RCC->PLL3FRACR & RCC_PLL3FRACR_FRACLE) >> RCC_PLL3FRACR_FRACLE_Pos;

   fracn1us = (pll3fracen * ((RCC->PLL3FRACR & RCC_PLL3FRACR_FRACV) >>
                             RCC_PLL3FRACR_FRACV_Pos));
   fracn1   = (float32_t)fracn1us;
   ...
   P_Div =
       ((RCC->PLL3CFGR2 & RCC_PLL3CFGR2_DIVP) >> RCC_PLL3CFGR2_DIVP_Pos) + 1U;
   P_Freq                        = pll3vco / (float32_t)P_Div;
   PLL3_Clocks->PLL3_P_Frequency = (uint32_t)P_Freq;

   Q_Div =
       ((RCC->PLL3CFGR2 & RCC_PLL3CFGR2_DIVQ) >> RCC_PLL3CFGR2_DIVQ_Pos) + 1U;
   Q_Freq                        = pll3vco / (float32_t)Q_Div;
   PLL3_Clocks->PLL3_Q_Frequency = (uint32_t)Q_Freq;

   R_Div =
       ((RCC->PLL3CFGR2 & RCC_PLL3CFGR2_DIVR) >> RCC_PLL3CFGR2_DIVR_Pos) + 1U;
   R_Freq                        = pll3vco / (float32_t)R_Div;
   PLL3_Clocks->PLL3_R_Frequency = (uint32_t)R_Freq;
}

/**
 * @brief  Returns the PLL4 clock frequencies :PLL4_P_Frequency,PLL4_R_Frequency
 * and PLL4_Q_Frequency
 ...
 *         right PLL4CLK value. Otherwise, any configuration based on this
 * function will be incorrect.
...
__weak void HAL_RCC_GetPLL4ClockFreq(PLL4_ClocksTypeDef *PLL4_Clocks)
{
   ...
   pll4m =
       ((RCC->PLL4CFGR1 & RCC_PLL4CFGR1_DIVM4) >> RCC_PLL4CFGR1_DIVM4_Pos) + 1U;
   pll4fracen =
       (RCC->PLL4FRACR & RCC_PLL4FRACR_FRACLE) >> RCC_PLL4FRACR_FRACLE_Pos;

   fracn1us = (pll4fracen * ((RCC->PLL4FRACR & RCC_PLL4FRACR_FRACV) >>
                             RCC_PLL4FRACR_FRACV_Pos));
   fracn1   = (float32_t)fracn1us;
   ...
   P_Div =
       ((RCC->PLL4CFGR2 & RCC_PLL4CFGR2_DIVP) >> RCC_PLL4CFGR2_DIVP_Pos) + 1U;
   P_Freq                        = pll4vco / (float32_t)P_Div;
   PLL4_Clocks->PLL4_P_Frequency = (uint32_t)P_Freq;

   Q_Div =
       ((RCC->PLL4CFGR2 & RCC_PLL4CFGR2_DIVQ) >> RCC_PLL4CFGR2_DIVQ_Pos) + 1U;
   Q_Freq                        = pll4vco / (float32_t)Q_Div;
   PLL4_Clocks->PLL4_Q_Frequency = (uint32_t)Q_Freq;

   R_Div =
       ((RCC->PLL4CFGR2 & RCC_PLL4CFGR2_DIVR) >> RCC_PLL4CFGR2_DIVR_Pos) + 1U;
 ...
 *         right PCLK6 value. Otherwise, any configuration based on this
 * function will be incorrect.
 ...
 *         right PCLK4 value. Otherwise, any configuration based on this
 * function will be incorrect.
 ...
 *         right PCLK5 value. Otherwise, any configuration based on this
 * function will be incorrect.
 ...
 *         right ACLK value. Otherwise, any configuration based on this function
 * will be incorrect.
 ...
 *         right HCLK5 value. Otherwise, any configuration based on this
 * function will be incorrect.
 ...
 *         right HCLK6 value. Otherwise, any configuration based on this
 * function will be incorrect.
 ...
 *         right HCLK1 value. Otherwise, any configuration based on this
 * function will be incorrect.
 ...
 *         right HCLK3 value. Otherwise, any configuration based on this
 * function will be incorrect.
 ...
 *         right HCLK4 value. Otherwise, any configuration based on this
 * function will be incorrect.
 ...
 *         right MLHCLK value. Otherwise, any configuration based on this
 * function will be incorrect.
 * @retval HCLK4 frequency
 */
 ...
 *         right MLAHB value. Otherwise, any configuration based on this
 * function will be incorrect.
 ...
 *         right FCLK value. Otherwise, any configuration based on this function
 * will be incorrect.
 ...
 *         right CKPER value. Otherwise, any configuration based on this
 * function will be incorrect.
   ...
   switch (__HAL_RCC_GET_MPU_SOURCE()) {
      ...
      case RCC_MPUSOURCE_MPUDIV:
         ...
         if (mpudiv == RCC_MPU_DIV_OFF) {
            ...
            HAL_RCC_GetPLL1ClockFreq(&pll1_clocks);
            mpussfreq =
                (pll1_clocks.PLL1_P_Frequency >> 16); /* divided by 16 */
         }
         break;
      ...
      case RCC_MPUSOURCE_HSE: mpussfreq = HSE_VALUE; break;
...
uint32_t HAL_RCC_GetAXISSFreq()
{
   ...
   switch (__HAL_RCC_GET_AXIS_SOURCE()) {
      ...
      case RCC_AXISSOURCE_HSE: axissfreq = HSE_VALUE; break;
...
uint32_t HAL_RCC_GetMLAHBSSFreq()
{
   ...
   switch (__HAL_RCC_GET_MLAHB_SOURCE()) {
      ...
      case RCC_MLAHBSSOURCE_HSE: mlahbssfreq = HSE_VALUE; break;

      case RCC_MLAHBSSOURCE_CSI: mlahbssfreq = CSI_VALUE; break;
