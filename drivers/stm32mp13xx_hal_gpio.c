/**
  ...
  ******************************************************************************
  ...
  The external interrupt/event controller consists of up to 23 edge detectors
        (16 lines are connected to GPIO) for generating event/interrupt requests
  (each input line can be independently configured to select the type (interrupt
  or event) and the corresponding trigger event (rising or falling or both).
   ...
   while (((GPIO_Init->Pin) >> position) != 0x00U) {
      ...
      if (iocurrent != 0x00U) {
         ...
         if (((GPIO_Init->Mode & GPIO_MODE) == MODE_OUTPUT) ||
            ...
            temp = GPIOx->OSPEEDR;
            temp &= ~(GPIO_OSPEEDR_OSPEEDR0
                      << (position * GPIO_OSPEEDR_OSPEEDR1_Pos));
            temp |=
                (GPIO_Init->Speed << (position * GPIO_OSPEEDR_OSPEEDR1_Pos));
            GPIOx->OSPEEDR = temp;
         ...
         if ((GPIO_Init->Mode & GPIO_MODE) == MODE_AF) {
            ...
            temp &=
                ~(GPIO_AFRL_AFR0 << ((position & 0x07U) * GPIO_AFRL_AFR1_Pos));
            temp |= ((GPIO_Init->Alternate)
                     << ((position & 0x07U) * GPIO_AFRL_AFR1_Pos));
         ...
         temp |= ((GPIO_Init->Mode & GPIO_MODE)
                  << (position * GPIO_MODER_MODER1_Pos));
         GPIOx->MODER = temp;
         ...
         if ((GPIO_Init->Mode & EXTI_MODE) != 0x00U) {
            ...
            temp &= ~(EXTI_EXTICR1_EXTI0
                      << (EXTI_EXTICR1_EXTI1_Pos * (position & 0x03U)));
...
void HAL_GPIO_DeInit(GPIO_TypeDef *GPIOx, uint32_t GPIO_Pin)
{
   ...
   while ((GPIO_Pin >> position) != 0x00U) {
      ...
      if (iocurrent != 0x00U) {
         ...
         GPIOx->MODER |=
             (GPIO_MODER_MODER0 << (position * GPIO_MODER_MODER1_Pos));
         ...
         GPIOx->PUPDR &=
             ~(GPIO_PUPDR_PUPDR0 << (position * GPIO_PUPDR_PUPDR1_Pos));
         /* Configure the default value IO Output Type */
         GPIOx->OTYPER &= ~(GPIO_OTYPER_OT0 << position);

         /* Configure the default value for IO Speed */
         GPIOx->OSPEEDR &=
             ~(GPIO_OSPEEDR_OSPEEDR0 << (position * GPIO_OSPEEDR_OSPEEDR1_Pos));
      }
      position++;
   }
}
