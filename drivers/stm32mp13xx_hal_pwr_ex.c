...
charged through an internal resistance. VBAT charging can be performed either
through a 5 KOhm resistor or through a 1.5 KOhm resistor.
      (+) VBAT charging is enabled by
HAL_PWREx_EnableBatteryCharging(ResistorValue) function with:
       (++) ResistorValue:
...
two threshold levels: VBAThigh and VBATlow. VBATH flag and VBATL flag in the PWR
control register 2 (PWR_CR2), indicate if VBAT is higher or lower than the
threshold.
      (+) The temperature can be monitored by comparing it with two threshold
levels, TEMPhigh and TEMPlow. TEMPH and TEMPL flags, in the PWR control register
2 (PWR_CR2), indicate whether the device temperature is higher or lower than the
threshold.
...
HAL_PWREx_DisableMonitoring() function.
      (+) The HAL_PWREx_GetVBATLevel() function return the VBAT level which can
be: PWR_VBAT_BELOW_LOW_THRESHOLD or PWR_VBAT_ABOVE_HIGH_THRESHOLD or
          PWR_VBAT_BETWEEN_HIGH_LOW_THRESHOLD.
      (+) The HAL_PWREx_GetTemperatureLevel() function return the Temperature
level which can be: PWR_TEMP_BELOW_LOW_THRESHOLD or
PWR_TEMP_ABOVE_HIGH_THRESHOLD or PWR_TEMP_BETWEEN_HIGH_LOW_THRESHOLD.

    *** AVD configuration ***
    =========================
    [..]
      (+) The AVD is used to monitor the VDDA power supply by comparing it to a
          threshold selected by the AVD Level (ALS[3:0] bits in the PWR_CSR1
register).
...
shall be enabled through  HAL_PWREx_EnableUSBVoltageDetector() function and
disabled by HAL_PWREx_DisableUSBVoltageDetector() function.
 ...
 * @note   After reset PWR_CR2 register is write-protected and the DBP bit in
 * the PWR control register 1 (PWR_CR1) has to be set before it can be written.
 ...
 *         When VDD is present, charge the external battery through an internal
 * resistor.
...
/**
 * @brief  Configure the analog voltage threshold detected by the Analog Voltage
 * Detector(AVD).
 * @param  sConfigAVD: pointer to an PWR_AVDTypeDef structure that contains the
 * configuration information for the AVD.
 * @note   Refer to the electrical characteristics of your device datasheet for
 * more details about the voltage threshold corresponding to each detection
 * level.
 * @retval None
 */
