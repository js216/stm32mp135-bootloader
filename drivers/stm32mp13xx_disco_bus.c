...
static int32_t I2C1_WriteReg(uint16_t DevAddr, uint16_t MemAddSize,
                             uint16_t Reg, uint8_t *pData, uint16_t Length);
static int32_t I2C1_ReadReg(uint16_t DevAddr, uint16_t MemAddSize, uint16_t Reg,
                            uint8_t *pData, uint16_t Length);
                          ...
                          uint16_t Length)
{
   ...
   if (I2C5_WriteReg(DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
                         ...
                         uint16_t Length)
{
   ...
   if (I2C5_ReadReg(DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
      if (HAL_I2C_GetError(&hbus_i2c5) == HAL_I2C_ERROR_AF) {
                            ...
                            uint16_t Length)
{
   ...
   if (I2C5_WriteReg(DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
      if (HAL_I2C_GetError(&hbus_i2c5) == HAL_I2C_ERROR_AF) {
                           ...
                           uint16_t Length)
{
   ...
   if (I2C5_ReadReg(DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
      if (HAL_I2C_GetError(&hbus_i2c5) == HAL_I2C_ERROR_AF) {
                          ...
                          uint16_t Length)
{
   ...
   if (I2C4_WriteReg(DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
                         ...
                         uint16_t Length)
{
   ...
   if (I2C4_ReadReg(DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
      if (HAL_I2C_GetError(&hbus_i2c4) == HAL_I2C_ERROR_AF) {
                            ...
                            uint16_t Length)
{
   ...
   if (I2C4_WriteReg(DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
      if (HAL_I2C_GetError(&hbus_i2c4) == HAL_I2C_ERROR_AF) {
                           ...
                           uint16_t Length)
{
   ...
   if (I2C4_ReadReg(DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
      if (HAL_I2C_GetError(&hbus_i2c4) == HAL_I2C_ERROR_AF) {
                          ...
                          uint16_t Length)
{
   ...
   if (I2C1_WriteReg(DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
                         ...
                         uint16_t Length)
{
   ...
   if (I2C1_ReadReg(DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
      if (HAL_I2C_GetError(&hbus_i2c1) == HAL_I2C_ERROR_AF) {
                            ...
                            uint16_t Length)
{
   ...
   if (I2C1_WriteReg(DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
      if (HAL_I2C_GetError(&hbus_i2c1) == HAL_I2C_ERROR_AF) {
                           ...
                           uint16_t Length)
{
   ...
   if (I2C1_ReadReg(DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length) ==
       (int32_t)HAL_OK) {
      ret = BSP_ERROR_NONE;
   } else {
      if (HAL_I2C_GetError(&hbus_i2c1) == HAL_I2C_ERROR_AF) {
...
int32_t BSP_I2C1_RegisterDefaultMspCallbacks(void)
{
   ...
   /* Register default MspInit/MspDeInit Callback */
   if (HAL_I2C_RegisterCallback(&hbus_i2c1, HAL_I2C_MSPINIT_CB_ID,
                                I2C1_MspInit) != HAL_OK) {
      ret = BSP_ERROR_PERIPH_FAILURE;
   } else if (HAL_I2C_RegisterCallback(&hbus_i2c1, HAL_I2C_MSPDEINIT_CB_ID,
                                       I2C1_MspDeInit) != HAL_OK) {
      ret = BSP_ERROR_PERIPH_FAILURE;
   } else {
      IsI2c1MspCbValid = 1U;
   }
 ...
 * @retval BSP status
 */
static int32_t I2C1_WriteReg(uint16_t DevAddr, uint16_t Reg,
                             uint16_t MemAddSize, uint8_t *pData,
                             uint16_t Length)
{
   if (HAL_I2C_Mem_Write(&hbus_i2c1, DevAddr, Reg, MemAddSize, pData, Length,
                         10000) == HAL_OK) {
      return BSP_ERROR_NONE;
   }
 ...
 * @retval BSP status
 */
static int32_t I2C1_ReadReg(uint16_t DevAddr, uint16_t Reg, uint16_t MemAddSize,
                            uint8_t *pData, uint16_t Length)
{
   if (HAL_I2C_Mem_Read(&hbus_i2c1, DevAddr, Reg, MemAddSize, pData, Length,
                        10000) == HAL_OK) {
      return BSP_ERROR_NONE;
   }
