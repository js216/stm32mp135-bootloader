 ...
 ===============================================================================
    ...
    *** Low Power modes configuration ***
    ...
    [..]
      ...
      (+) CSleep mode: MPU/MCU clocks stopped and the MPU/MCU sub-system
allocated peripheral(s) clocks operate according to RCC PERxLPEN
      ...
      (+) LP-Stop mode: bus matrix clocks stalled, the oscillators can be
stopped. VDDCORE is supplied. To enter into this mode:
                        - Both MPU and MCU sub-systems are in CStop or CStandby.
                        - The LPDS bit (PWR_CR1) selects LP-Stop.
                        - The LVDS bit (PWR_CR1) selects normal voltage.
                        - At least one PDDS bit (PWR_MPUCR/PWR_MCUCR) selects
Stop.
      (+) LPLV-Stop mode: bus matrix clocks stalled, the oscillators can be
stopped. VDDCORE may be supplied at a lower level. To enter into this mode:
                          - Both MPU and MCU sub-systems are in CStop or
CStandby.
...
Stop.
      (+) Standby mode: System powered down.
                        To enter into this mode:
                        - MPU sub-system is in CStandby or CStop with CSTBYDIS =
1 and MCU subsystem is in CStop.
   ...
   *** CSleep mode ***
   ==================
    [..]
      (+) Entry:
        The CSleep mode is entered by using the
HAL_PWR_EnterSLEEPMode(Regulator, SLEEPEntry) functions with:
        (++) STOPEntry:
      ...
      (+) Exit:
          MCU: Any peripheral interrupt acknowledged by the nested vectored
interrupt controller (NVIC) if entered by WFI or any event if entered by WFE
   ...
   *** CStop mode ***
   =================
    [..]
      (+) Entry:
         The CStop mode is entered using the HAL_PWR_EnterSTOP(ModeRegulator,
STOPEntry) function with:
      ...
      (+) Exit:
          MCU: Any EXTI Line (Internal or External) configured in
Interrupt/Event mode depending of entry mode. MPU: Any EXTI Line (Internal or
External) configured in Interrupt mode

   *** MPU CStandby mode / MCU CStop allowing Standby mode ***
   ====================
    [..]
    (+)
      The Standby mode allows to achieve the lowest power consumption. On
Standby mode the voltage regulator is disabled. The PLLs, the HSI oscillator and
      ...
      (++) Exit:
        ...
        (+++) If system is in Standby mode wake up is generated by reset
through: WKUP pins, RTC alarm (Alarm A and Alarm B), RTC wakeup, tamper event,
 ...
 *         This parameter can be one of the following legacy values, which sets
 * the default polarity: detection on high level (rising edge):
 *           @arg PWR_WAKEUP_PIN1, PWR_WAKEUP_PIN2, PWR_WAKEUP_PIN3,
 * PWR_WAKEUP_PIN4, PWR_WAKEUP_PIN5, PWR_WAKEUP_PIN6 or one of the following
 * value where the user can explicitly states the enabled pin and the chosen
 * polarity
...
void HAL_PWR_EnableWakeUpPin(uint32_t WakeUpPinPolarity)
{
   ...
   MODIFY_REG(PWR->WKUPCR, clear_mask,
              (WakeUpPinPolarity & (PWR_WKUPCR_WKUPP | PWR_WKUPCR_WKUPPUPD)));
   /* Modify polarity and pull configuration */
   SET_BIT(PWR->WKUPCR,
           (WakeUpPinPolarity & PWR_WKUPCR_WKUPC)); /* Clear wake up flag */
   SET_BIT(PWR->MPUWKUPENR,
           (WakeUpPinPolarity &
            PWR_WAKEUP_PIN_MASK)); /* Enable the Wake up pin for CPU1 */
#endif                             /* CORE_CA7 */
}
...
void HAL_PWR_EnableWakeUpPinIT(uint32_t WakeUpPinx)
{
   ...
   switch ((WakeUpPinx & PWR_WAKEUP_PIN_MASK)) {
      case PWR_WAKEUP_PIN1: __HAL_WKUP_EXTI_ENABLE_IT(EXTI_IMR2_IM55); break;

      case PWR_WAKEUP_PIN2: __HAL_WKUP_EXTI_ENABLE_IT(EXTI_IMR2_IM56); break;

      case PWR_WAKEUP_PIN3: __HAL_WKUP_EXTI_ENABLE_IT(EXTI_IMR2_IM57); break;

      case PWR_WAKEUP_PIN4: __HAL_WKUP_EXTI_ENABLE_IT(EXTI_IMR2_IM58); break;

      case PWR_WAKEUP_PIN5: __HAL_WKUP_EXTI_ENABLE_IT(EXTI_IMR2_IM59); break;

      case PWR_WAKEUP_PIN6: __HAL_WKUP_EXTI_ENABLE_IT(EXTI_IMR2_IM60); break;
...
void HAL_PWR_DisableWakeUpPinIT(uint32_t WakeUpPinx)
{
   assert_param(IS_PWR_WAKEUP_PIN(WakeUpPinx));
   switch ((WakeUpPinx & PWR_WAKEUP_PIN_MASK)) {
      case PWR_WAKEUP_PIN1: __HAL_WKUP_EXTI_DISABLE_IT(EXTI_IMR2_IM55); break;

      case PWR_WAKEUP_PIN2: __HAL_WKUP_EXTI_DISABLE_IT(EXTI_IMR2_IM56); break;

      case PWR_WAKEUP_PIN3: __HAL_WKUP_EXTI_DISABLE_IT(EXTI_IMR2_IM57); break;

      case PWR_WAKEUP_PIN4: __HAL_WKUP_EXTI_DISABLE_IT(EXTI_IMR2_IM58); break;

      case PWR_WAKEUP_PIN5: __HAL_WKUP_EXTI_DISABLE_IT(EXTI_IMR2_IM59); break;

      case PWR_WAKEUP_PIN6: __HAL_WKUP_EXTI_DISABLE_IT(EXTI_IMR2_IM60); break;
 ...
 *
 * @note In CSleep mode, the systick is stopped to avoid exit from this mode
 * with systick interrupt when used as time base for Timeout
 *
 ...
 * families This parameter can be one of the following values:
 *            @arg PWR_MAINREGULATOR_ON: CSLEEP mode with regulator ON
 *            @arg PWR_LOWPOWERREGULATOR_ON: CSLEEP mode with low power
 * regulator ON
 * @param SLEEPEntry: Specifies if CSLEEP mode in entered with WFI or WFE
 ...
 *       to be the only one IT having this value before calling
 * HAL_PWR_EnterSTOPMode. Make sure RCC_WAKEUP_IRQn is the only one IT allowed
 * to wake up core before calling  HAL_PWR_EnterSTOPMode (BASEPRI) Reestablish
 * priority level once system is completely waken up (clock restore and IO
 * compensation)
 ...
 * ON
 * @param STOPEntry: Specifies if CStop mode is entered with WFI or WFE
 * instruction. This parameter can be one of the following values:
 ...
 * processor re-enters CSleep mode when an interruption handling is over.
