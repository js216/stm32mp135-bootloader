 ...
 */
typedef struct {
   uint32_t PageSize; /*!< NAND memory page (without spare area) size measured
                         in bytes for 8 bits addressing or words for 16 bits
                         addressing             */
   ...
   uint32_t
       BlockSize; /*!< NAND memory block size measured in number of pages */
   ...
   uint32_t
       PlaneSize; /*!< NAND memory zone size measured in number of blocks */
 ...
 */
typedef struct {
   uint8_t Offset; /*!< Offset of the ECC from the beginning of the spare area
                      measured in bytes for 8 bits addressing or words for 16
                      bits addressing */
 ...
 */
typedef struct {
   uint16_t CorrectibleErrorTotal; /*!< Total number of correctible errors found
                                      during the last page read */
   uint8_t
       CorrectibleErrorMax; /*!< Maximum number of correctible errors found in a
                               single sector during the last page read */
   uint8_t CorrectibleSectorCount; /*!< Number of sector with at least one
                                      correctible error found during the last
                                      page read */
   uint8_t BadSectorCount; /*!< Number of sector with too many errors found
                              during the last page read */
...
#endif /* USE_HAL_NAND_REGISTER_CALLBACKS  */
{
...
#ifdef HAL_MDMA_MODULE_ENABLED
   NAND_SequencerConfigTypeDef
       SequencerConfig; /*!< NAND sequencer configuration */
   ...
   uint32_t DeviceAddress; /*!< Device address for the current bank and chip
                              select   */
...
HAL_StatusTypeDef HAL_NAND_ECC_SetBCHMode(NAND_HandleTypeDef *hnand,
                                          uint32_t Mode);
HAL_StatusTypeDef HAL_NAND_ECC_SetAccessMode(NAND_HandleTypeDef *Device,
                                             uint32_t Mode);
...
/** @addtogroup NAND_Exported_Functions_Group5 Input and Output functions with
 * error correction
 * @{
 */
...
HAL_StatusTypeDef
HAL_NAND_Sequencer_Read_Page_16b(NAND_HandleTypeDef *hnand,
                                 NAND_AddressTypeDef *pAddress,
                                 uint16_t *pBuffer);
HAL_StatusTypeDef
HAL_NAND_Sequencer_Write_Page_16b(NAND_HandleTypeDef *hnand,
                                  NAND_AddressTypeDef *pAddress,
                                  uint16_t *pBuffer);
...
HAL_StatusTypeDef
HAL_NAND_Sequencer_ECC_Read_Page_16b(NAND_HandleTypeDef *hnand,
                                     NAND_AddressTypeDef *pAddress,
                                     uint16_t *pBuffer);
HAL_StatusTypeDef
HAL_NAND_Sequencer_ECC_Write_Page_16b(NAND_HandleTypeDef *hnand,
                                      NAND_AddressTypeDef *pAddress,
                                      uint16_t *pBuffer);
