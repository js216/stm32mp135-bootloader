...
#define EXTI_CONFIG_OFFSET                                                     \
   0x08U /* 0x20: offset between CPU Rising/Falling                            \
                                         ...
                                         EXTI_ConfigTypeDef *pExtiConfig)
{
   ...
   /* Set Interrupt And Event Mask for Core if configuration given into
    * parameter mode */
                                         ...
                                         EXTI_ConfigTypeDef *pExtiConfig)
{
   ...
   if ((pExtiConfig->Line & EXTI_CONFIG) != 0x00U) {
      ...
      if ((pExtiConfig->Line & EXTI_GPIO) == EXTI_GPIO) {
         ...
         regval = EXTI->EXTICR[(linepos >> 2U) & 0x03UL];
         pExtiConfig->GPIOSel =
             ((regval << (EXTI_EXTICR1_EXTI1_Pos *
                          (((32u / EXTI_EXTICR1_EXTI1_Pos) - 1U) -
                           (linepos & 0x03u)))) >>
   ...
   if ((hexti->Line & EXTI_CONFIG) != 0x00U) {
      ...
      *regaddr = regval;
      ...
      if ((hexti->Line & EXTI_GPIO) == EXTI_GPIO) {
         ...
         regval = EXTI->EXTICR[(linepos >> 2U) & 0x03UL];
         regval &= ~(EXTI_EXTICR1_EXTI0
                     << (EXTI_EXTICR1_EXTI1_Pos * (linepos & 0x03UL)));
         EXTI->EXTICR[(linepos >> 2U) & 0x03UL] = regval;
      }
   }
                                            ...
                                            void (*pPendingCbfn)(void))
{
   ...
   switch (CallbackID) {
      ...
      case HAL_EXTI_RISING_CB_ID: hexti->RisingCallback = pPendingCbfn; break;

      case HAL_EXTI_FALLING_CB_ID: hexti->FallingCallback = pPendingCbfn; break;
