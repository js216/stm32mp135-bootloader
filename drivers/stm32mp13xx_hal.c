...
#define __STM32MP13xx_HAL_VERSION                                              \
   ((__STM32MP13xx_HAL_VERSION_MAIN << 24) |                                   \
    (__STM32MP13xx_HAL_VERSION_SUB1 << 16) |                                   \
    (__STM32MP13xx_HAL_VERSION_SUB2 << 8) | (__STM32MP13xx_HAL_VERSION_RC))
 ...
 * other HAL function), it performs the following: Configures the SysTick to
 * generate an interrupt each 1 millisecond, which is clocked by the HSI (at
 * this stage, the clock is not yet configured and thus the system is running
 * from the internal HSI at 64 MHz). Set NVIC Group Priority to 4. Calls the
 * HAL_MspInit() callback function defined in user file "stm32mp13xx_hal_msp.c"
...
HAL_StatusTypeDef HAL_Init(void)
{
    ...
    * after Reset is HSI) */
   if (HAL_InitTick(PRIO_TICK) != HAL_OK) {
      return HAL_ERROR;
   }
 ...
 * be taken if HAL_Delay() is called from a peripheral ISR process, The the
 * SysTick interrupt must have higher priority (numerically lower) than the
 * peripheral interrupt. Otherwise the caller ISR process will be blocked. The
 * function is declared as __weak  to be overwritten  in case of other
...
__weak HAL_StatusTypeDef HAL_InitTick(uint32_t TickPriority)
{
    ...
    */
   /* TickPriority is based on 16 level priority (from MCUs) so set it in 7:4
    * and leave bit 3=0 */
...
{
   /* Defining uwTick as local static to avoid MISRA warnings as it is only
   being used here right now In case another function uses it define it as
   extern in hal.h and then reuse. Earlier implementation declares it as extern
   in hal.h and defined it as global in this file */
 ...
 * HAL_SuspendTick() is called, the the SysTick interrupt will be disabled and
 * so Tick increment is suspended.
 * @note This function is declared as __weak to be overwritten in case of other
 *       implementations in user file.
 * @retval None
 */
 ...
 * @note In the default implementation , SysTick timer is the source of time
 * base. It is used to generate interrupts at regular time intervals. Once
 * HAL_ResumeTick() is called, the the SysTick interrupt will be enabled and so
 ...
 *   @arg SYSCFG_ETH_RGMII : Select the reduced gigabit media-independent
 * interface
...
/**
 * @brief  Enables the booster to reduce the total harmonic distortion of the
 * analog switch when the supply voltage is lower than 2.7 V.
 * @note   Activating the booster allows to guaranty the analog switch AC
 * performance when the supply voltage is below 2.7 V: in this case, the analog
 * switch performance is the same on the full voltage range
 * @retval None
 */
 ...
 * @note   This bit is active only if PRODUCT_BELOW_25V user option bit is set.
 * It must be used only if the product supply voltage is below 2.5 V. Setting
 * this bit when VDD is higher than 2.5 V might be destructive.
...
{
   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_TRACE_SIGNAL) ==
       SYSCFG_HIGHSPEED_TRACE_SIGNAL) {
      WRITE_REG(SYSCFG->HSLVEN0R, 0x1018U);
   }

   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_QUADSPI_SIGNAL) ==
       SYSCFG_HIGHSPEED_QUADSPI_SIGNAL) {
      WRITE_REG(SYSCFG->HSLVEN1R, 0x1018U);
   }

   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_ETH1_SIGNAL) ==
       SYSCFG_HIGHSPEED_ETH1_SIGNAL) {
      WRITE_REG(SYSCFG->HSLVEN2R, 0x1018U);
   }

   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_ETH2_SIGNAL) ==
       SYSCFG_HIGHSPEED_ETH2_SIGNAL) {
      WRITE_REG(SYSCFG->HSLVEN3R, 0x1018U);
   }

   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_SDMMC1_SIGNAL) ==
       SYSCFG_HIGHSPEED_SDMMC1_SIGNAL) {
      WRITE_REG(SYSCFG->HSLVEN4R, 0x1018U);
   }

   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_SDMMC2_SIGNAL) ==
       SYSCFG_HIGHSPEED_SDMMC2_SIGNAL) {
      WRITE_REG(SYSCFG->HSLVEN5R, 0x1018U);
   }

   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_SPI1_SIGNAL) ==
       SYSCFG_HIGHSPEED_SPI1_SIGNAL) {
      WRITE_REG(SYSCFG->HSLVEN6R, 0x1018U);
   }

   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_SPI2_SIGNAL) ==
       SYSCFG_HIGHSPEED_SPI2_SIGNAL) {
      WRITE_REG(SYSCFG->HSLVEN7R, 0x1018U);
   }

   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_SPI3_SIGNAL) ==
       SYSCFG_HIGHSPEED_SPI3_SIGNAL) {
      WRITE_REG(SYSCFG->HSLVEN8R, 0x1018U);
   }

   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_SPI4_SIGNAL) ==
       SYSCFG_HIGHSPEED_SPI4_SIGNAL) {
      WRITE_REG(SYSCFG->HSLVEN9R, 0x1018U);
   }

   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_SPI5_SIGNAL) ==
       SYSCFG_HIGHSPEED_SPI5_SIGNAL) {
      WRITE_REG(SYSCFG->HSLVEN10R, 0x1018U);
   }

   if ((SYSCFG_HighSpeedSignal & SYSCFG_HIGHSPEED_LTDC_SIGNAL) ==
       SYSCFG_HIGHSPEED_LTDC_SIGNAL) {
 ...
 * @param  SYSCFG_CompCode: Selects the code to be applied for the I/O
 * compensation cell This parameter can be one of the following values:
 *   @arg SYSCFG_CELL_CODE : Select Code from the cell (available in the
 * SYSCFG_CMPCRx)
 *   @arg SYSCFG_REGISTER_CODE: Select Code from RANSRC[3:0] and RAPSRC[3:0]
 * register values
 * @param CompCells: correspond to Compensation cells.
 * This parameter can be one or a combination of the following values:
 *   @arg SYSCFG_MAIN_COMP_CELL
                                       ...
                                       uint32_t SYSCFG_CompCode)
{
   ...
   if ((CompCells & SYSCFG_MAIN_COMP_CELL) == SYSCFG_MAIN_COMP_CELL) {
      MODIFY_REG(SYSCFG->CMPCR, SYSCFG_CMPCR_SW_CTRL,
                 (uint32_t)(SYSCFG_CompCode));
   }

   if ((CompCells & SYSCFG_SD1_COMP_CELL) == SYSCFG_SD1_COMP_CELL) {
      MODIFY_REG(SYSCFG->CMPSD1CR, SYSCFG_CMPSD1CR_SW_CTRL,
                 (uint32_t)(SYSCFG_CompCode));
   }

   if ((CompCells & SYSCFG_SD2_COMP_CELL) == SYSCFG_SD2_COMP_CELL) {
      MODIFY_REG(SYSCFG->CMPSD2CR, SYSCFG_CMPSD2CR_SW_CTRL,
                 (uint32_t)(SYSCFG_CompCode));
   }
}
 ...
 *         This code is applied to the I/O compensation cell when the CS bit of
 * the SYSCFG_CMPCR is set
                                       ...
                                       uint32_t SYSCFG_NMOSCode)
{
   ...
   if ((CompCell & SYSCFG_MAIN_COMP_CELL) == SYSCFG_MAIN_COMP_CELL) {
      MODIFY_REG(SYSCFG->CMPCR, SYSCFG_CMPCR_RANSRC | SYSCFG_CMPCR_RAPSRC,
                 (((uint32_t)(SYSCFG_PMOSCode) << 20) |
                  ((uint32_t)(SYSCFG_NMOSCode) << 16)));
   }

   else if ((CompCell & SYSCFG_SD1_COMP_CELL) == SYSCFG_SD1_COMP_CELL) {
      MODIFY_REG(SYSCFG->CMPSD1CR,
                 SYSCFG_CMPSD1CR_RANSRC | SYSCFG_CMPSD1CR_RAPSRC,
                 (((uint32_t)(SYSCFG_PMOSCode) << 20) |
                  ((uint32_t)(SYSCFG_NMOSCode) << 16)));
   }

   else if ((CompCell & SYSCFG_SD2_COMP_CELL) == SYSCFG_SD2_COMP_CELL) {
      MODIFY_REG(SYSCFG->CMPSD2CR,
                 SYSCFG_CMPSD2CR_RANSRC | SYSCFG_CMPSD2CR_RAPSRC,
                 (((uint32_t)(SYSCFG_PMOSCode) << 20) |
                  ((uint32_t)(SYSCFG_NMOSCode) << 16)));
   }
...
void HAL_SYSCFG_DisableIOCompensation(uint32_t CompCells)
{
   ...
   if ((CompCells & SYSCFG_MAIN_COMP_CELL) == SYSCFG_MAIN_COMP_CELL) {
       ...
       */
      HAL_SYSCFG_CompensationCodeConfig(SYSCFG_MAIN_COMP_CELL, pmos_val,
                                        nmos_val);
   }
   ...
   if ((CompCells & SYSCFG_SD1_COMP_CELL) == SYSCFG_SD1_COMP_CELL) {
      ...
      /* Copy actual value of APSRC[3:0]/ANSRC[3:0] in RAPSRC[3:0]/RANSRC[3:0]
       */
      HAL_SYSCFG_CompensationCodeConfig(SYSCFG_SD1_COMP_CELL, pmos_val,
                                        nmos_val);
   }
   ...
   if ((CompCells & SYSCFG_SD2_COMP_CELL) == SYSCFG_SD2_COMP_CELL) {
      ...
      /* Copy actual value of APSRC[3:0]/ANSRC[3:0] in RAPSRC[3:0]/RANSRC[3:0]
       */
      HAL_SYSCFG_CompensationCodeConfig(SYSCFG_SD2_COMP_CELL, pmos_val,
                                        nmos_val);
   }
...
HAL_StatusTypeDef HAL_SYSCFG_EnableIOCompensation(uint32_t CompCells)
{
   ...
   __IO uint32_t count =
       SYSCFG_DEFAULT_TIMEOUT * (SystemCoreClock / 20U / 1000U);
