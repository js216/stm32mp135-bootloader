/**
  ...
  ******************************************************************************
  ...
  should be called from the DMAMUX IRQ handler i.e DMAMUX1_OVR_IRQHandler or
  DMAMUX2_OVR_IRQHandler . As only one interrupt line is available for all
  DMAMUX channels and request generators , HAL_DMA_MUX_IRQHandler should be
  called with, as parameter, the appropriate DMA handle as many as used DMAs in
  the user project (exception done if a given DMA is not using the DMAMUX SYNC
  block neither a request generator)
  ...
  stream is enabled.
     -@-  Multi (Double) buffer mode is only possible with D2 DMAs i.e DMA1 or
  DMA2. not BDMA. Multi (Double) buffer mode is not possible with D3 BDMA.
...
@verbatim
 ...
 ===============================================================================
    [..]  This section provides functions allowing to:
      ...
      (+) Configure the DMA_MUX Synchronization Block using
HAL_DMAEx_ConfigMuxSync function.
      (+) Configure the DMA_MUX Request Generator Block using
HAL_DMAEx_ConfigMuxRequestGenerator function.
      (+) Functions HAL_DMAEx_EnableMuxRequestGenerator and
HAL_DMAEx_DisableMuxRequestGenerator can then be used to respectively
enable/disable the request generator.
      (+) Handle DMAMUX interrupts using HAL_DMAEx_MUX_IRQHandler : should be
called from the DMAMUX IRQ handler i.e DMAMUX1_OVR_IRQHandler or
                        ...
                        HAL_DMA_MuxSyncConfigTypeDef *pSyncConfig)
{
   ...
   if (hdma->State == HAL_DMA_STATE_READY) {
      ...
      /* Disable the synchronization and event generation before applying a new
       * config */
    ...
    HAL_DMA_MuxRequestGeneratorConfigTypeDef *pRequestGeneratorConfig)
{
   ...
   if (IS_DMA3_INSTANCE(hdma)) {
      assert_param(
          IS_DMAMUX2_REQUEST_GEN_SIGNAL_ID(pRequestGeneratorConfig->SignalID));
   } else {
      assert_param(
          IS_DMAMUX1_REQUEST_GEN_SIGNAL_ID(pRequestGeneratorConfig->SignalID));
   }
   ...
   */
   if (hdma->DMAmuxRequestGen == 0U) {
      ...
      /* RequestGenerator must be disable prior to the configuration i.e GE bit
       * is 0 */
