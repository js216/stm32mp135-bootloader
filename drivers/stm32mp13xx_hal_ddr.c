...
typedef enum { BASE_DDRCTRL, BASE_DDRPHYC, BASE_NONE } base_type;
...
#define DDRCTL_REG(x, y)                                                       \
   {.offset = offsetof(DDRCTRL_TypeDef, x), .par_offset = offsetof(y, x)}

#define DDRPHY_REG(x, y)                                                       \
   {.offset = offsetof(DDRPHYC_TypeDef, x), .par_offset = offsetof(y, x)}
...
#define DDRCTRL_STAT_OPERATING_MODE_SR                                         \
   (DDRCTRL_STAT_OPERATING_MODE_0 | DDRCTRL_STAT_OPERATING_MODE_1)
#define DDRCTRL_STAT_OPERATING_MODE_NORMAL DDRCTRL_STAT_OPERATING_MODE_0
#define DDRCTRL_STAT_SELFREF_TYPE_ASR                                          \
   (DDRCTRL_STAT_SELFREF_TYPE_0 | DDRCTRL_STAT_SELFREF_TYPE_1)
...
#define DDRCTRL_DBG_Q_AND_DATA_PIPELINE_EMPTY                                  \
   (DDRCTRL_DBGCAM_DBG_WR_Q_EMPTY | DDRCTRL_DBGCAM_DBG_RD_Q_EMPTY |            \
    DDRCTRL_DBGCAM_DATA_PIPELINE_EMPTY)
#define DDRCTRL_DBGCAM_DBG_Q_DEPTH                                             \
   (DDRCTRL_DBGCAM_DBG_WR_Q_EMPTY | DDRCTRL_DBGCAM_DBG_LPR_Q_DEPTH |           \
...
{
#ifdef DDR_DUAL_AXI_PORT
   SET_BIT(RCC->DDRITFCR, RCC_DDRITFCR_DDRC1EN | RCC_DDRITFCR_DDRC2EN |
                              RCC_DDRITFCR_DDRPHYCEN |
                              RCC_DDRITFCR_DDRPHYCAPBEN |
                              RCC_DDRITFCR_DDRCAPBEN);
#else  /* DDR_DUAL_AXI_PORT */
   SET_BIT(RCC->DDRITFCR, RCC_DDRITFCR_DDRC1EN | RCC_DDRITFCR_DDRPHYCEN |
                              RCC_DDRITFCR_DDRPHYCAPBEN |
                              RCC_DDRITFCR_DDRCAPBEN);
#endif /* DDR_DUAL_AXI_PORT */
}
...
static HAL_StatusTypeDef self_refresh_zcal(uint32_t zdata)
{
   ...
   MODIFY_REG(DDRPHYC->ZQ0CR0, DDRPHYC_ZQ0CR0_ZDATA_Msk,
...
static int32_t disable_host_interface(void)
{
   ...
   do {
      ...
      count++;
             ...
             DDRCTRL_DBG_Q_AND_DATA_PIPELINE_EMPTY) ||
            (count < 2U));
...
static HAL_StatusTypeDef mode_register_write(uint32_t addr, uint32_t data)
{
    ...
    */
   mrctrl0 =
       DDRCTRL_MRCTRL0_MR_TYPE_WRITE | DDRCTRL_MRCTRL0_MR_RANK_ALL |
       ((addr << DDRCTRL_MRCTRL0_MR_ADDR_Pos) & DDRCTRL_MRCTRL0_MR_ADDR_Msk);
...
static HAL_StatusTypeDef ddr3_dll_off(void)
{
    ...
    */
   mr1 &= ~(DDRPHYC_MR1_RTT0 | DDRPHYC_MR1_RTT1 | DDRPHYC_MR1_RTT2);
...
static HAL_StatusTypeDef HAL_DDR_Refresh_Restore(uint32_t rfshctl3,
                                                 uint32_t pwrctl)
{
   HAL_StatusTypeDef ret;
...
static HAL_StatusTypeDef refresh_compensation(uint32_t start)
{
   ...
   trefi =
       ((READ_REG(DDRCTRL->RFSHTMG) & DDRCTRL_RFSHTMG_T_RFC_NOM_X1_X32_Msk) >>
...
static int32_t ddr_sw_self_refresh_in(void)
{
   ...
   MODIFY_REG(DDRPHYC->ACIOCR, DDRPHYC_ACIOCR_CKPDD_Msk,
              DDRPHYC_ACIOCR_CKPDD_0);
   MODIFY_REG(DDRPHYC->ACIOCR, DDRPHYC_ACIOCR_CKPDR_Msk,
...
#ifdef DDR_DUAL_AXI_PORT
   CLEAR_BIT(RCC->DDRITFCR, RCC_DDRITFCR_DDRC1EN | RCC_DDRITFCR_DDRC2EN |
                                RCC_DDRITFCR_DDRCAPBEN |
                                RCC_DDRITFCR_DDRPHYCAPBEN);
#else  /* DDR_DUAL_AXI_PORT */
   CLEAR_BIT(RCC->DDRITFCR, RCC_DDRITFCR_DDRC1EN | RCC_DDRITFCR_DDRCAPBEN |
                                RCC_DDRITFCR_DDRPHYCAPBEN);
#endif /* DDR_DUAL_AXI_PORT */
...
static uint32_t ddr_test_addr_bus(void)
{
   ...
   for (testoffset = sizeof(uint32_t); (testoffset & addressmask) != 0U;
      ...
      for (offset = sizeof(uint32_t); (offset & addressmask) != 0U;
           offset <<= 1) {
         if ((READ_REG(
                  *(volatile uint32_t *)(DDR_BASE_ADDR + (uint32_t)offset)) !=
              DDR_PATTERN) &&
             (offset != testoffset)) {
            return (uint32_t)(DDR_BASE_ADDR + offset);
         }
...
static uint32_t ddr_check_size(void)
{
   ...
   while (offset < DDR_MAX_SIZE) {
      WRITE_REG(*(volatile uint32_t *)(DDR_BASE_ADDR + offset),
                DDR_ANTIPATTERN);
      __DSB();
...
static HAL_DDR_SelfRefreshModeTypeDef ddr_sr_read_mode(void)
{
   ...
   switch (pwrctl & (DDRCTRL_PWRCTL_EN_DFI_DRAM_CLK_DISABLE |
      ...
      case 0U: ret = HAL_DDR_SW_SELF_REFRESH_MODE; break;
      ...
      case DDRCTRL_PWRCTL_EN_DFI_DRAM_CLK_DISABLE | DDRCTRL_PWRCTL_SELFREF_EN:
         ...
         break;
      default: ret = HAL_DDR_INVALID_MODE; break;
   }
...
HAL_StatusTypeDef HAL_DDR_Init(DDR_InitTypeDef *iddr)
{
   ...
   if (iddr->self_refresh) {
      ...
      pir &= ~(DDRPHYC_PIR_ZCAL & DDRPHYC_PIR_DRAMRST & DDRPHYC_PIR_DRAMINIT);
...
HAL_StatusTypeDef HAL_DDR_SR_SetMode(HAL_DDR_SelfRefreshModeTypeDef mode)
{
   ...
   switch (mode) {
      case HAL_DDR_SW_SELF_REFRESH_MODE: ret = ddr_sr_mode_ssr(); break;
      case HAL_DDR_AUTO_SELF_REFRESH_MODE: ret = ddr_sr_mode_asr(); break;
      case HAL_DDR_HW_SELF_REFRESH_MODE: ret = ddr_sr_mode_hsr(); break;
      default: ret = HAL_ERROR; break;
   }
