...
#define BSP_PMIC_PWRCTRL_PIN_Assert()                                          \
   HAL_GPIO_WritePin(PMIC_PWRCTRL_PORT, PMIC_PWRCTRL_PIN, GPIO_PIN_RESET);
#define BSP_PMIC_PWRCTRL_PIN_Pull()                                            \
   HAL_GPIO_WritePin(PMIC_PWRCTRL_PORT, PMIC_PWRCTRL_PIN, GPIO_PIN_SET);
...
#define OTP_LDO4_FORCED                                                        \
   ((NVM_SECTOR3_REGISTER_0 & 0x08U) >> 3) // Otp_ldo4_forced
#define OTP_LONGKEYPRESSED                                                     \
   ((NVM_SECTOR3_REGISTER_0 & 0x04U) >> 2) // nvm_longkeypress
 ...
 *
 */
#define OTP_CLAMP_OUTPUT_BUCK4                                                 \
   ((NVM_SECTOR3_REGISTER_3 & 0x80U) >> 7) // nvm_clamp_output_buck4
#define OTP_BYPASS_MODE_LDO3                                                   \
   ((NVM_SECTOR3_REGISTER_3 & 0x40U) >> 6) // nvm_bypass_mode_ldo3
#define OTP_RANK_VREFDDR                                                       \
   ((NVM_SECTOR3_REGISTER_3 & 0x30U) >> 4) // nvm_rank_vrefddr
 ...
 *
 */
#define OTP_OUTPUT_BUCK4                                                       \
   ((NVM_SECTOR3_REGISTER_4 & 0xC0U) >> 6) // nvm_output_buck4
#define OTP_OUTPUT_BUCK3                                                       \
   ((NVM_SECTOR3_REGISTER_4 & 0x30U) >> 4) // nvm_output_buck3
#define OTP_OUTPUT_BUCK2                                                       \
   ((NVM_SECTOR3_REGISTER_4 & 0x0CU) >> 2)                  // nvm_output_buck2
 ...
 * [7]	OTP_SWOFF_BY_BOOST_OVP:
 ...
 01:  2.5V
 10:  3.3V
 11:  output_buck2<4:0>/2 (VTT termination for DDR3 x32, Analog divider
 ...
 *
 */
#define OTP_SWOFF_BY_BOOST_OVP                                                 \
   ((NVM_SECTOR3_REGISTER_5 & 0x80U) >> 7) // OTP_SWOFF_BY_BOOST_OVP
#define OTP_OUTPUT_LDO3                                                        \
   ((NVM_SECTOR3_REGISTER_5 & 0x30U) >> 4) // nvm_output_ldo3
#define OTP_OUTPUT_LDO2                                                        \
   ((NVM_SECTOR3_REGISTER_5 & 0x0CU) >> 2)                 // nvm_output_ldo2
...
#define OTP_OUTPUT_LDO6                                                        \
   ((NVM_SECTOR3_REGISTER_6 & 0x0CU) >> 2)                 // nvm_output_ldo6
...
static I2C_HandleTypeDef hpmic_i2c4;
...
static uint16_t buck4_voltage_table[] = {
    600,  625,  650,  675,  700,  725,  750,  775,  800,  825,  850,
    875,  900,  925,  950,  975,  1000, 1025, 1050, 1075, 1100, 1125,
    1150, 1175, 1200, 1225, 1250, 1275, 1300, 1300, 1350,
    1350, // 31  1,35
    1400, // 32  1,40
    1400, // 33  1,40
    1450, // 34  1,45
...
static uint8_t STPMU1_Voltage_Find_Index(PMIC_RegulId_TypeDef id,
                                         uint16_t milivolts);
...
void STPMU1_Enable_Interrupt(PMIC_IRQn IRQn)
{
   ...
   /* value to be set in IRQ register corresponds to BIT_PMIC(7-N) where N is
    * the Interrupt id modulo 8 */
   ...
   for (irq_reg = 0U; irq_reg < STM32_PMIC_NUM_IRQ_REGS; irq_reg++) {
      ...
      for (i = 0U; i < 8U; i++) {
         if ((latch_events & (1U << i)) != 0U) {
            ...
            STPMU1_INTn_Callback(
                (PMIC_IRQn)(uint8_t)(((irq_reg * 8U) + (7U - i))));
         }
      }
...
void STPMU1_Sw_Reset(void)
{
   ...
   STPMU1_Register_Update((uint8_t)MAIN_CONTROL_REG, (uint8_t)1U,
                          (uint8_t)SOFTWARE_SWITCH_OFF_ENABLED);
}
...
void STPMU1_Regulator_Enable(PMIC_RegulId_TypeDef id)
{
   ...
   STPMU1_Register_Update((uint8_t)regul->control_reg, (uint8_t)BIT_PMIC(0U),
                          (uint8_t)BIT_PMIC(0U));
}
...
void STPMU1_Regulator_Disable(PMIC_RegulId_TypeDef id)
{
   ...
   STPMU1_Register_Update((uint8_t)regul->control_reg, (uint8_t)0U,
                          (uint8_t)BIT_PMIC(0U));
}
...
uint8_t STPMU1_Register_Read(uint8_t register_id)
{
   ...
   status = (uint32_t)BSP_I2C4_ReadReg(STPMU1_I2C_ADDRESS,
                                       (uint16_t)register_id, &Value, 1U);
...
void STPMU1_Register_Write(uint8_t register_id, uint8_t value)
{
   ...
   status = (uint32_t)BSP_I2C4_WriteReg(STPMU1_I2C_ADDRESS,
                                        (uint16_t)register_id, &value, 1U);
...
static uint32_t BSP_PMIC_MspInit(I2C_HandleTypeDef *hi2c)
{
   ...
   ENTER_CRITICAL_SECTION(PMIC_INTn_PORT);
   HAL_GPIO_Init(PMIC_INTn_PORT, &GPIO_InitStruct);
   EXIT_CRITICAL_SECTION(PMIC_INTn_PORT);

   /* Enable and set INTn EXTI Interrupt  */
#if defined(CORE_CA7)
   IRQ_SetPriority((IRQn_ID_t)EXTI8_IRQn, PRIO_STPMIC);
   IRQ_Enable((IRQn_ID_t)EXTI8_IRQn);
#endif
...
static uint32_t BSP_PMIC_MspDeInit(I2C_HandleTypeDef *hi2c)
{
   ...
   uint32_t status = BSP_ERROR_NONE;
   /*##-1- Reset I2C Clock / Disable peripherals and GPIO
    * Clocks###############*/
   status = (uint32_t)BSP_I2C4_DeInit();
    ...
    * ##########################################*/
#if defined(CORE_CA7)
   IRQ_Disable((IRQn_ID_t)EXTI8_IRQn);
#endif
   ENTER_CRITICAL_SECTION(PMIC_INTn_PORT);
...
uint32_t BSP_PMIC_Is_Device_Ready(void)
{
   ...
   if ((uint32_t)BSP_I2C4_IsReady(STPMU1_I2C_ADDRESS, 1) !=
       (uint32_t)BSP_ERROR_NONE) {
      status = (uint32_t)BSP_ERROR_BUSY;
   }
   return status;
}

/* Use Xls I2C COnfiguration Tools with I2C Clock config + output clocks
 * requirement */
...
{
   uint32_t status = BSP_ERROR_NONE;
   if (HAL_I2C_GetState(&hpmic_i2c4) != HAL_I2C_STATE_RESET) {
      /* Deinit the I2C */
      BSP_PMIC_MspDeInit(&hpmic_i2c4);
   }
   return status;
}
...
{
   STPMU1_Register_Write(BUCK1_PWRCTRL_REG,
                         STPMU1_Register_Read(BUCK1_CONTROL_REG));
   STPMU1_Register_Write(BUCK2_PWRCTRL_REG,
                         STPMU1_Register_Read(BUCK2_CONTROL_REG));
   STPMU1_Register_Write(BUCK3_PWRCTRL_REG,
                         STPMU1_Register_Read(BUCK3_CONTROL_REG));
   STPMU1_Register_Write(BUCK4_PWRCTRL_REG,
                         STPMU1_Register_Read(BUCK4_CONTROL_REG));
   STPMU1_Register_Write(LDO1_PWRCTRL_REG,
                         STPMU1_Register_Read(LDO1_CONTROL_REG));
   STPMU1_Register_Write(LDO2_PWRCTRL_REG,
                         STPMU1_Register_Read(LDO2_CONTROL_REG));
   STPMU1_Register_Write(LDO3_PWRCTRL_REG,
                         STPMU1_Register_Read(LDO3_CONTROL_REG));
   STPMU1_Register_Write(LDO4_PWRCTRL_REG,
                         STPMU1_Register_Read(LDO4_CONTROL_REG));
   STPMU1_Register_Write(LDO5_PWRCTRL_REG,
                         STPMU1_Register_Read(LDO5_CONTROL_REG));
   STPMU1_Register_Write(LDO6_PWRCTRL_REG,
                         STPMU1_Register_Read(LDO6_CONTROL_REG));
   STPMU1_Register_Write(VREF_DDR_PWRCTRL_REG,
                         STPMU1_Register_Read(VREF_DDR_CONTROL_REG));
