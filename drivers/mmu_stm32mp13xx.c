 ...
 * @brief   CMSIS Cortex-A7 Device Peripheral Access Layer MMU Setup source
 * file.
 *
...
static uint32_t Sect_Normal_Shared; // outer & inner wb/wa, shareable,
...
static uint32_t
    Sect_Device_RW_Shared; // as Sect_Device_RO, but writeable, shareable

static uint32_t Page_L1_4k = 0x0;   // generic
static uint32_t Page_4k_Normal_Cod; // outer & inner wb/wa        , shareable,
                                    // executable,     ro, domain 0
static uint32_t
    Page_4k_Normal_Cod_NonCacheable; // outer & inner non-cacheable, shareable,
                                     // executable,     ro, domain 0
static uint32_t Page_4k_Normal_RO;   // outer & inner wb/wa        , shareable,
                                     // non-executable, ro, domain 0
static uint32_t
    Page_4k_Normal_RO_NonCacheable; // outer & inner non-cacheable, shareable,
                                    // non-executable, ro, domain 0
static uint32_t Page_4k_Normal_RW;  // outer & inner wb/wa        , shareable,
                                    // non-executable, rw, domain 0
static uint32_t
    Page_4k_Normal_RW_NonCacheable; // outer & inner non-cacheable, shareable,
                                    // non-executable, rw, domain 0
...
void MMU_CreateTranslationTable(void)
{
...
#endif
   volatile uint32_t code_and_data_table_l2_base_4k =
       ((uint32_t)ttb_addr + TTB_L1_SIZE);
   volatile uint32_t sram_table_l2_base_4k =
       (code_and_data_table_l2_base_4k + TTB_L2_1M_SIZE);
   volatile uint32_t gic_table_l2_base_4k =
       (sram_table_l2_base_4k + TTB_L2_1M_SIZE);
   volatile uint32_t sysram_table_l2_base_4k =
       (gic_table_l2_base_4k + TTB_L2_1M_SIZE);

   // Create faulting entries for full 4GB address space to ensure any access to
   // incorrect memory location will be trapped Further sections and pages setup
   // will overwrite these default table entries
   ...
   region.xn_t         = EXECUTE;
   /* This should be READ but SW4STM32/GDB/OpenOCD is not able to step in a
   ...
   MMU_TTSection(ttb_addr, (uint32_t)AXI_BUS_MEMORY_BASE,
                 ((QSPI_MEM_BASE - AXI_BUS_MEMORY_BASE) >> 20), Sect_Device_RO);

   // Define 256 QSPI memory as device, non-shareable, non-executable, ro
   MMU_TTSection(ttb_addr, (uint32_t)QSPI_MEM_BASE,
                 ((FMC_NAND_MEM_BASE - QSPI_MEM_BASE) >> 20), Sect_Device_RO);
   ...
   // All DDR (1GB) Executable, Cacheable & RW - applications may choose to
   // divide memory into RO executable
   ...
   MMU_TTPage4k(ttb_addr, SYSRAM_BASE & PAGE_1MB_ALIGN_MASK, 1024U / 4U,
                Page_L1_4k, (uint32_t *)sysram_table_l2_base_4k,
                DESCRIPTOR_FAULT);
   // Create (32 * 4k)=128k Normal entries to cover full SYSRAM, if the
   // executable is in SYSRAM, part of this table will be overwritten
   MMU_TTPage4k(ttb_addr, SYSRAM_BASE, 128U / 4U, Page_L1_4k,
                (uint32_t *)sysram_table_l2_base_4k,
                Page_4k_Normal_RW_NonCacheable);

   //-------------------- SRAM ------------------
   // Create (256 * 4k)=1MB faulting entries to cover SRAM 1M aligned range
   MMU_TTPage4k(ttb_addr, AHB_SRAM, 1024U / 4U, Page_L1_4k,
                (uint32_t *)sram_table_l2_base_4k, DESCRIPTOR_FAULT);
   // Create (8 * 4k)=32k Normal entries to cover full SRAM
   MMU_TTPage4k(ttb_addr, AHB_SRAM, 32U / 4U, Page_L1_4k,
                (uint32_t *)sram_table_l2_base_4k,
                Page_4k_Normal_RW_NonCacheable);

   //--------------------- PERIPHERALS -------------------
   // Define Peripheral memory as Strongly ordered, shareable, RW , Non
   // Cacheable
   MMU_TTSection(ttb_addr, (uint32_t)PERIPH_BASE,
                 ((AXI_BUS_MEMORY_BASE - PERIPH_BASE) >> 20), Sect_SO);

   //-------------------- GIC ------------------
   // Create (256 * 4k)=1MB faulting entries to cover GIC 1M aligned range
   MMU_TTPage4k(ttb_addr, GIC_BASE & PAGE_1MB_ALIGN_MASK, 1024U / 4U,
                Page_L1_4k, (uint32_t *)gic_table_l2_base_4k, DESCRIPTOR_FAULT);
   // Create (7 * 4k)=28k Device entries to cover full GIC
   MMU_TTPage4k(ttb_addr, GIC_BASE, 28U / 4U, Page_L1_4k,
                (uint32_t *)gic_table_l2_base_4k, Page_4k_Device_RW);

   //-------------------- Code and data ------------------
   if ((uint32_t)text_start_addr >= DRAM_MEM_BASE) {
      /* Code is in DDR */

      // Ensure Code and RO data can fit in single 1M section because this is
      // the size of Level to code_and_data_table_l2_base_4k Table. If it does
      // not fit in a single 1 MB page, then we do not add specific protection
      // for code and data because we would need one more L2 table to do so. To
      // add this new table, this would require to change linker scripts of all
      // examples. So, if the executable is larger than 1M, its code and
      // read-only areas are both writable and executable like the remaining of
      // the DRAM.
      uint32_t aligned_CodeStart =
          (uint32_t)text_start_addr & PAGE_1MB_ALIGN_MASK;
      uint32_t aligned_RO_Data_End =
          ((uint32_t)rodata_end_addr + PAGE_1MB_MASK) & PAGE_1MB_ALIGN_MASK;
      if (aligned_RO_Data_End - aligned_CodeStart <= PAGE_1MB_SIZE) {
         // Create (256 * 4k)=1MB RW entries to cover 1M aligned range
         MMU_TTPage4k(ttb_addr, aligned_CodeStart, 1024U / 4U, Page_L1_4k,
                      (uint32_t *)code_and_data_table_l2_base_4k,
                      Page_4k_Normal_RW);

         // Create Normal, executable+RO entries to cover Code
         pageNum =
             ((uint32_t)text_end_addr - (uint32_t)text_start_addr + 4095) /
             4096;
         MMU_TTPage4k(ttb_addr, (uint32_t)text_start_addr, pageNum, Page_L1_4k,
                      (uint32_t *)code_and_data_table_l2_base_4k,
                      Page_4k_Normal_Cod);

         // Create Normal, non-executable+RO entries to cover RO data range
         pageNum =
             ((uint32_t)rodata_end_addr - (uint32_t)rodata_start_addr + 4095) /
             4096;
         MMU_TTPage4k(ttb_addr, (uint32_t)rodata_start_addr, pageNum,
                      Page_L1_4k, (uint32_t *)code_and_data_table_l2_base_4k,
                      Page_4k_Normal_RO);
      }
   } else {
      /* Code is in SYSRAM */

      // Create (256 * 4k)=1MB faulting entries to cover 1M aligned range (this
      // needs to be done again because this is not the same L2 page table)
      MMU_TTPage4k(ttb_addr, SYSRAM_BASE & PAGE_1MB_ALIGN_MASK, 1024U / 4U,
                   Page_L1_4k, (uint32_t *)code_and_data_table_l2_base_4k,
                   DESCRIPTOR_FAULT);

      // Create (32 * 4k)=128k Normal entries to cover full SYSRAM (this needs
      // to be done again because this is not the same L2 page table)
      MMU_TTPage4k(ttb_addr, SYSRAM_BASE, 128U / 4U, Page_L1_4k,
                   (uint32_t *)code_and_data_table_l2_base_4k,
                   Page_4k_Normal_RW_NonCacheable);
      ...
      // Create Normal, executable+RO entries to cover Code
      pageNum =
          ((uint32_t)text_end_addr - (uint32_t)text_start_addr + 4095U) / 4096U;
      MMU_TTPage4k(ttb_addr, (uint32_t)text_start_addr, pageNum, Page_L1_4k,
                   (uint32_t *)code_and_data_table_l2_base_4k,
                   Page_4k_Normal_Cod);
      ...
      // Create Normal, non-executable+RO entries to cover RO data range
      pageNum =
          ((uint32_t)rodata_end_addr - (uint32_t)rodata_start_addr + 4095U) /
          4096U;
      MMU_TTPage4k(ttb_addr, (uint32_t)rodata_start_addr, pageNum, Page_L1_4k,
                   (uint32_t *)code_and_data_table_l2_base_4k,
                   Page_4k_Normal_RO_NonCacheable);
   }

   /* Set location of level 1 page table
   ; 31:14 - Translation table base addr (31:14-TTBCR.N, TTBCR.N is 0 out of
   reset) ; 13:7  - 0x0 ; 6     - IRGN[0] 0x0 (Inner WB WA) ; 5     - NOS 0x0
   (Non-shared) ; 4:3   - RGN     0x1 (Outer WB WA) ; 2     - IMP     0x0
   (Implementation Defined) ; 1     - S       0x0 (Non-shared) ; 0     - IRGN[1]
   0x1 (Inner WB WA) */
