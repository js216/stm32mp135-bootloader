...
/*
 * ID 1023. This value is returned to a processor, in response to an interrupt
 * acknowledge, if there is no pending interrupt with sufficient priority for it
 * to be signaled to the processor.
 */
#define GIC_ACKNOWLEDGE_RESPONSE 1023U

/*  GICC_IAR bit assignments
 * [31:13] - Reserved.
 * [12:10] CPUID For SGIs in a multiprocessor implementation, this field
 * identifies the processor that requested the interrupt. It returns the number
 * of the CPU interface that made the request, for example a value of 3 means
 * the request was generated by a write to the GICD_SGIR on CPU interface 3. For
 * all other interrupts this field is RAZ. [9:0] Interrupt ID The interrupt ID
 */
#define GICC_IAR_VALID_BITMASK  0x3FF
#define GICC_IAR_CPU_ID_BITMASK 0x1C00
#define GICC_IAR_CPU_ID_SHIFT   10

#define __GIC_GET_CPU_ID(__IAR)                                                \
   (((__IAR) & GICC_IAR_CPU_ID_BITMASK) >> GICC_IAR_CPU_ID_SHIFT)
...
static uint32_t GetPLL1PClockFreq(void)
{
   ...
   fracn1 =
       ((float_t)(uint32_t)(pll1fracen *
                            ((RCC->PLL1FRACR & RCC_PLL1FRACR_FRACV) >> 3)));
   pll1vco =
       ((float_t)(uint32_t)(((RCC->PLL1CFGR1 & RCC_PLL1CFGR1_DIVN) + 1U)) +
        (fracn1 / (float_t)0x1FFF));

   if (pll1m != 0U) {
      switch (pllsource) {
         case 0x00: /* HSI used as PLL clock source */
            pll1vco *= ((float_t)(uint32_t)(HSI_VALUE >> (RCC->HSICFGR &
                                                          RCC_HSICFGR_HSIDIV)) /
                        (float_t)pll1m);
            break;
         ...
         case 0x02: /* No clock source for PLL */ pll1vco = (float_t)0; break;

         default: break;
      }
      pll1p_freq = (((uint32_t)(float_t)pll1vco) /
                    (uint32_t)((RCC->PLL1CFGR2 & RCC_PLL1CFGR2_DIVP) + 1U));
   } else {
      pll1p_freq = 0U;
   }
   return pll1p_freq;
...
*/
  ...
  __ASM volatile("MRC p15, 0, r3, c14, c2, 0\n"
   ...
   /* C version with IRQ processing time compensation.
      ...
      Same result as Assembly version when optimized because functions are
      inlined and generated code is same as assembly version Result is not good
      when not optimized because "inline" functions are not inlined by GCC when
      optimization is off. So time for functions calls between Counter read and
      counter write are not compensated

      Compensates for IRQ processing time (hardware and software) by adding
      current counter value which decrements below 0 after IRQ trigger and thus
      contains -(time since IRQ triggered)
...
__irq __arm void irq_handler(void)
{
   ...
   while (1U) {
      ...
      if (ItId <= GIC_HIGHEST_INTERRUPT_VALUE) /* Highest value of GIC Valid
                                                  Interrupt */
      {
         ...
         /* Normal case: whenever there is no more pending IRQ , IAR returns
          * ACKNOWLEDGE special IRQ value */
