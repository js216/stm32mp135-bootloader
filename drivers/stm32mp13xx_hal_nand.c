/**
  ...
  ******************************************************************************
  ...
  implementations, it should be implemented separately.
    ...
    *** Callback registration ***
    ...
    [..]
      ...
      Use Functions @ref HAL_NAND_RegisterCallback() to register a user
  ...
  ID and a pointer to the user callback function.
      ...
      Use function @ref HAL_NAND_UnRegisterCallback() to reset a callback to the
  ...
  Callback ID.
      ...
      By default, after the @ref HAL_NAND_Init and if the state is
  HAL_NAND_STATE_RESET all callbacks are reset to the corresponding legacy weak
  (surcharged) functions. Exception done for MspInit and MspDeInit callbacks
  that are respectively reset to the legacy weak (surcharged) functions in the
  @ref HAL_NAND_Init and @ref  HAL_NAND_DeInit only when these callbacks are
  null (not registered beforehand). If not, MspInit or MspDeInit are not null,
  the @ref HAL_NAND_Init and @ref HAL_NAND_DeInit keep and use the user
  ...
  HAL_NAND_RegisterCallback before calling @ref HAL_NAND_DeInit or @ref
  HAL_NAND_Init function.
...
static uint32_t HAL_NAND_ComputeWordsToBytes(uint32_t ColumnAddressInWords,
                                             uint32_t MemoryDataWidth)
{
   return (MemoryDataWidth == NAND_BUS_WIDTH_8) ? ColumnAddressInWords
                                                : ColumnAddressInWords * 2U;
}

static void HAL_NAND_WriteCommand(NAND_HandleTypeDef *restrict hnand,
                                  uint8_t Data)
{
...
static void HAL_NAND_WriteAddress(NAND_HandleTypeDef *restrict hnand,
                                  uint8_t Data)
{
   *(__IO uint8_t *)((uint32_t)(hnand->DeviceAddress | NAND_ADDR_OFFSET)) =
       Data;
   __DSB();
}
...
static HAL_StatusTypeDef HAL_NAND_WaitReady(NAND_HandleTypeDef *hnand,
                                            uint32_t TimeOut)
{
   uint32_t nandStatus;
   /* Get tick */
...
static void HAL_NAND_SetAddress(NAND_HandleTypeDef *restrict hnand,
                                uint32_t NandAddress, uint32_t ColumnAddress)
{
   uint32_t totalPageNumber = hnand->Config.BlockSize * hnand->Config.BlockNbr;
   uint32_t columnAddressInBytes =
       HAL_NAND_ComputeWordsToBytes(ColumnAddress, hnand->Init.MemoryDataWidth);
...
static HAL_StatusTypeDef HAL_NAND_ReadBegin(NAND_HandleTypeDef *restrict hnand,
...
static void HAL_NAND_WriteBegin(NAND_HandleTypeDef *restrict hnand,
                                uint32_t NandAddress, uint32_t ColumnAddress,
                                uint32_t Area)
{
   /* The following line is not needed for recent NANDs. */
   HAL_NAND_WriteCommand(hnand,
                         (Area == (uint32_t)NAND_AREA_SPARE) &&
                                 (hnand->Config.PageSize <= NAND_PAGE_SIZE_512)
                             ? NAND_CMD_AREA_C
                             : NAND_CMD_AREA_A);
...
static void HAL_NAND_ChangeReadColumn(NAND_HandleTypeDef *restrict hnand,
                                      uint32_t ColumnAddress)
{
   uint32_t columnAddressInBytes =
       HAL_NAND_ComputeWordsToBytes(ColumnAddress, hnand->Init.MemoryDataWidth);
...
static void HAL_NAND_ChangeWriteColumn(NAND_HandleTypeDef *restrict hnand,
                                       uint32_t ColumnAddress)
{
   uint32_t columnAddressInBytes =
       HAL_NAND_ComputeWordsToBytes(ColumnAddress, hnand->Init.MemoryDataWidth);
...
static void HAL_NAND_ReadFromDevice8(uint32_t DeviceAddress, uint32_t Size,
                                     uint8_t *pBuffer)
{
   uint32_t index;
...
static void HAL_NAND_ReadFromDevice16(uint32_t DeviceAddress, uint32_t Size,
                                      uint16_t *pBuffer)
{
   uint32_t index;
   for (index = 0U; index < Size; index++) {
...
static void HAL_NAND_ReadFromDevice(NAND_HandleTypeDef *restrict hnand,
                                    uint32_t Size, void *pBuffer,
                                    uint32_t Offset)
{
   if (hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8) {
      HAL_NAND_ReadFromDevice8(hnand->DeviceAddress, Size,
                               &((uint8_t *)pBuffer)[Offset]);
   } else {
      HAL_NAND_ReadFromDevice16(hnand->DeviceAddress, Size,
                                &((uint16_t *)pBuffer)[Offset]);
   }
}

static void HAL_NAND_WriteToDevice8(uint32_t DeviceAddress, uint32_t Size,
                                    uint8_t *pBuffer)
{
   uint32_t index;
   for (index = 0U; index < Size; index++) {
...
static void HAL_NAND_WriteToDevice16(uint32_t DeviceAddress, uint32_t Size,
                                     uint16_t *pBuffer)
{
   uint32_t index;
   for (index = 0U; index < Size; index++) {
...
static void HAL_NAND_WriteToDevice(NAND_HandleTypeDef *restrict hnand,
                                   uint32_t Size, void *pBuffer,
                                   uint32_t Offset)
{
   if (hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8) {
      HAL_NAND_WriteToDevice8(hnand->DeviceAddress, Size,
                              &((uint8_t *)pBuffer)[Offset]);
   } else {
      HAL_NAND_WriteToDevice16(hnand->DeviceAddress, Size,
                               &((uint16_t *)pBuffer)[Offset]);
   }
}
 ...
 * @retval HAL status
 */
static HAL_StatusTypeDef HAL_NAND_Read_Page(NAND_HandleTypeDef *hnand,
                                            ...
                                            uint32_t NumPagesToRead)
{
   ...
   if (hnand->State == HAL_NAND_STATE_READY) {
      ...
      /* Page(s) read loop */
      while ((numPagesRead < NumPagesToRead) &&
             (nandAddress < nandAddressMax)) {
         /* Begin reading */
         ...
         HAL_NAND_ReadFromDevice(hnand, hnand->Config.PageSize, pBuffer,
                                 bufferOffset);
 ...
 *                the configuration information for NAND module.
 * @param  pAddress  pointer to NAND address structure
 * @param  pBuffer  pointer to destination read buffer. pBuffer should be 16-bit
 * aligned
 * @param  NumPagesToRead  number of pages to read from block
 * @retval HAL status
 */
 ...
 * @retval HAL status
 */
static HAL_StatusTypeDef HAL_NAND_Read_SinglePage(NAND_HandleTypeDef *hnand,
                                                  NAND_AddressTypeDef *pAddress,
                                                  void *pBuffer,
                                                  uint32_t Offset,
                                                  uint32_t Length)
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Read_SinglePage_8b(NAND_HandleTypeDef *hnand,
                                              NAND_AddressTypeDef *pAddress,
                                              uint8_t *pBuffer, uint32_t Offset,
                                              uint32_t Length)
{
   assert(hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8);
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Read_SinglePage_16b(NAND_HandleTypeDef *hnand,
                                               NAND_AddressTypeDef *pAddress,
                                               uint16_t *pBuffer,
 ...
 * @retval HAL status
 */
static HAL_StatusTypeDef HAL_NAND_Write_Page(NAND_HandleTypeDef *hnand,
                                             NAND_AddressTypeDef *pAddress,
                                             void *pBuffer,
                                             uint32_t NumPagesToWrite)
{
   ...
   if (hnand->State == HAL_NAND_STATE_READY) {
      ...
      /* Page(s) write loop */
      while ((numPagesWritten < NumPagesToWrite) &&
             (nandAddress < nandAddressMax)) {
         /* Begin writing */
         HAL_NAND_WriteBegin(hnand, nandAddress, 0, NAND_AREA_USER);

         /* Write data to memory */
         HAL_NAND_WriteToDevice(hnand, hnand->Config.PageSize, pBuffer,
                                bufferOffset);
         bufferOffset += hnand->Config.PageSize;
 ...
 *                the configuration information for NAND module.
 * @param  pAddress  pointer to NAND address structure
 * @param  pBuffer  pointer to source buffer to write. pBuffer should be 16-bit
 * aligned
 * @param  NumPagesToWrite   number of pages to write to block
 * @retval HAL status
 */
...
HAL_NAND_Write_SinglePage(NAND_HandleTypeDef *hnand,
                          NAND_AddressTypeDef *pAddress, void *pBuffer,
                          uint32_t Offset, uint32_t Length)
{
   HAL_StatusTypeDef status = HAL_OK;
   uint32_t nandAddressMax;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Write_SinglePage_8b(NAND_HandleTypeDef *hnand,
                                               NAND_AddressTypeDef *pAddress,
                                               uint8_t *pBuffer,
                                               uint32_t Offset, uint32_t Length)
{
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Write_SinglePage_16b(NAND_HandleTypeDef *hnand,
                                                NAND_AddressTypeDef *pAddress,
                                                uint16_t *pBuffer,
                                                uint32_t Offset,
                                                uint32_t Length)
 ...
 * @retval HAL status
 */
static HAL_StatusTypeDef HAL_NAND_Read_SpareArea(NAND_HandleTypeDef *hnand,
                                                 NAND_AddressTypeDef *pAddress,
                                                 void *pBuffer,
                                                 uint32_t NumSpareAreasToRead)
{
   ...
   if (hnand->State == HAL_NAND_STATE_READY) {
      ...
      while ((numSpareAreaRead < NumSpareAreasToRead) &&
             (nandAddress < nandAddressMax)) {
         /* Begin reading */
         status = HAL_NAND_ReadBegin(hnand, nandAddress,
                                     SPARE_AREA_COLUMN_ADDRESS(hnand));
         if (status != HAL_OK) {
            break;
         }

         /* Get data into Buffer */
         HAL_NAND_ReadFromDevice(hnand, hnand->Config.SpareAreaSize, pBuffer,
                                 bufferOffset);
...
{
   assert(hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8);
   return HAL_NAND_Read_SpareArea(hnand, pAddress, pBuffer,
                                  NumSpareAreasToRead);
}
 ...
 */
static HAL_StatusTypeDef
HAL_NAND_Read_SingleSpareArea(NAND_HandleTypeDef *hnand,
                              NAND_AddressTypeDef *pAddress, void *pBuffer,
                              uint32_t Offset, uint32_t Length)
{
   HAL_StatusTypeDef status = HAL_OK;
   ...
   if (hnand->State == HAL_NAND_STATE_READY) {
      ...
      if (nandAddress < nandAddressMax) {
         /* Begin reading */
         status = HAL_NAND_ReadBegin(hnand, nandAddress,
                                     SPARE_AREA_COLUMN_ADDRESS(hnand) + Offset);
         if (status == HAL_OK) {
            /* Get data into Buffer */
            HAL_NAND_ReadFromDevice(hnand, Length, pBuffer, 0);
         }
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Read_SingleSpareArea_8b(
    NAND_HandleTypeDef *hnand, NAND_AddressTypeDef *pAddress, uint8_t *pBuffer,
    uint32_t Offset, uint32_t Length)
{
   assert(hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8);
   return HAL_NAND_Read_SingleSpareArea(hnand, pAddress, pBuffer, Offset,
                                        Length);
}

/**
 * @brief  Read in a single spare area from NAND memory block (16-bit
 * addressing)
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 * @param  pAddress  pointer to NAND address structure
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Read_SingleSpareArea_16b(
    NAND_HandleTypeDef *hnand, NAND_AddressTypeDef *pAddress, uint16_t *pBuffer,
    uint32_t Offset, uint32_t Length)
{
   assert(hnand->Init.MemoryDataWidth != NAND_BUS_WIDTH_8);
   return HAL_NAND_Read_SingleSpareArea(hnand, pAddress, pBuffer, Offset,
 ...
 * @retval HAL status
 */
static HAL_StatusTypeDef HAL_NAND_Write_SpareArea(NAND_HandleTypeDef *hnand,
                                                  NAND_AddressTypeDef *pAddress,
                                                  void *pBuffer,
                                                  uint32_t NumSpareAreasTowrite)
{
   ...
   if (hnand->State == HAL_NAND_STATE_READY) {
      ...
      while ((numSpareAreasWritten < NumSpareAreasTowrite) &&
             (nandAddress < nandAddressMax)) {
         /* Begin writing */
         HAL_NAND_WriteBegin(hnand, nandAddress,
                             SPARE_AREA_COLUMN_ADDRESS(hnand), NAND_AREA_SPARE);

         /* Write data to memory */
         HAL_NAND_WriteToDevice(hnand, hnand->Config.SpareAreaSize, pBuffer,
                                bufferOffset);
         bufferOffset += hnand->Config.SpareAreaSize;
...
{
   assert(hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8);
   return HAL_NAND_Write_SpareArea(hnand, pAddress, pBuffer,
                                   NumSpareAreasTowrite);
}
 ...
 */
static HAL_StatusTypeDef
HAL_NAND_Write_SingleSpareArea(NAND_HandleTypeDef *hnand,
                               NAND_AddressTypeDef *pAddress, void *pBuffer,
                               uint32_t Offset, uint32_t Length)
{
   HAL_StatusTypeDef status = HAL_OK;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Write_SingleSpareArea_8b(
    NAND_HandleTypeDef *hnand, NAND_AddressTypeDef *pAddress, uint8_t *pBuffer,
    uint32_t Offset, uint32_t Length)
{
   assert(hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8);
   return HAL_NAND_Write_SingleSpareArea(hnand, pAddress, pBuffer, Offset,
                                         Length);
}
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Write_SingleSpareArea_16b(
    NAND_HandleTypeDef *hnand, NAND_AddressTypeDef *pAddress, uint16_t *pBuffer,
    uint32_t Offset, uint32_t Length)
{
   assert(hnand->Init.MemoryDataWidth != NAND_BUS_WIDTH_8);
   ...
   if (hnand->State == HAL_NAND_STATE_READY) {
      switch (CallbackId) {
         ...
         case HAL_NAND_MSP_DEINIT_CB_ID:
            ...
            break;
         case HAL_NAND_IT_CB_ID: hnand->ItCallback = pCallback; break;
         default:
 ...
 *         NAND Callback is redirected to the weak (surcharged) predefined
 * callback
 * @param hnand : NAND handle
   ...
   if (hnand->State == HAL_NAND_STATE_READY) {
      switch (CallbackId) {
         ...
         case HAL_NAND_MSP_DEINIT_CB_ID:
            hnand->MspDeInitCallback = HAL_NAND_MspDeInit;
            break;
         case HAL_NAND_IT_CB_ID: hnand->ItCallback = HAL_NAND_ITCallback; break;
         default:
            /* update return status */
            status = HAL_ERROR;
            break;
 ...
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 * @param  Algorithm Algorithm to use for the ECC: FMC_NAND_ECC_ALGO_HAMMING or
 * FMC_NAND_ECC_ALGO_BCH
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_ECC_SetAlgorithm(NAND_HandleTypeDef *hnand,
                                            uint32_t Algorithm)
{
   HAL_StatusTypeDef status = HAL_OK;
 ...
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 * @param  Mode Mode to use for the BCH ECC: FMC_NAND_BCH_4BIT or
 * FMC_NAND_BCH_8BIT
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_ECC_SetBCHMode(NAND_HandleTypeDef *hnand,
                                          uint32_t Mode)
{
   HAL_StatusTypeDef status = HAL_OK;
...
/**
  ...
  * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
  *                the configuration information for NAND module.
  * @param  Mode Access mode to use for the ECC: FMC_NAND_ECC_ACCESS_MODE_READ
  or FMC_NAND_ECC_ACCESS_MODE_WRITE
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_NAND_ECC_SetAccessMode(NAND_HandleTypeDef *hnand,
                                             uint32_t Mode)
{
   HAL_StatusTypeDef status = HAL_OK;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_ECC_GetBCH(NAND_HandleTypeDef *hnand,
                                      uint32_t *ECCval, uint32_t Timeout)
{
   HAL_StatusTypeDef status;
 ...
 * - If too many errors were found (more than 4 or 8 depending of BCH
 * configuration), ErrorCount is set to -1
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_ECC_GetErrorsBCH(NAND_HandleTypeDef *hnand,
                                            uint16_t *Positions,
                                            int32_t *ErrorCount,
                                            uint32_t Timeout)
{
 ...
 * @retval NAND status
 */
HAL_StatusTypeDef HAL_NAND_Read_Status(NAND_HandleTypeDef *hnand,
                                       uint32_t *NandStatus)
{
   /* Process Locked */
   __HAL_LOCK(hnand);
...
/** @defgroup NAND_Exported_Functions_Group5 Input and Output functions with
error correction
  *  @brief   Peripheral State functions
  *
@verbatim
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_ECC_Init(NAND_HandleTypeDef *hnand,
                                    const NAND_EccConfigTypeDef *Config)
{
   ...
   if (hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8) {
      hnand->EccState.EccSize =
          (uint8_t)((hnand->EccState.EccLength + 7u) >> 3);
   } else {
      hnand->EccState.SectorSize >>= 1;
      hnand->EccState.EccSize =
          (uint8_t)((hnand->EccState.EccLength + 15u) >> 4);
   }

   hnand->EccState.SectorCount =
       (uint8_t)(hnand->Config.PageSize / hnand->EccState.SectorSize);
...
static void HAL_NAND_ECC_UpdateErrorCounters(NAND_HandleTypeDef *hnand,
                                             int32_t ErrorCount)
{
   if (ErrorCount > 0) {
...
static void HAL_NAND_ECC_flipBit(NAND_HandleTypeDef *hnand, void *Data,
                                 uint32_t DataOffset, uint32_t BitPosition)
{
   uint32_t byte = BitPosition >> 3;
   uint32_t bit  = BitPosition & 0x7u;
   uint8_t *data8 =
       (uint8_t *)Data + (hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8
                              ? DataOffset
                              : DataOffset * 2U);
   data8[byte] ^= (1u << bit);
}
 ...
 * @param EccLength length of the ECC bits in both EccWhenWriting and
 * EccWhenReading
 ...
 */
static int32_t
HAL_NAND_ECC_CorrectHamming(NAND_HandleTypeDef *hnand, void *SectorData,
                            uint32_t SectorOffset, uint32_t EccLength,
                            uint32_t EccWhenWriting, uint32_t EccWhenReading)
{
   ...
   do {
      switch (xoredEcc % 4u) {
         default: return -1;
         case 2: bitPosition += shifting; break;
         case 1: break;
      }
      shifting <<= 1;
      xoredEcc >>= 2;
 ...
 *         -1 if more than one error was detected (it cannot be corrected)
 */
static void HAL_NAND_ECC_CorrectBCH(NAND_HandleTypeDef *hnand, void *SectorData,
                                    uint32_t SectorOffset, int32_t ErrorCount,
                                    const uint16_t *Positions)
{
   int32_t errorIndex;
   uint32_t bitPosition;
   uint32_t sectorSizeInBits = hnand->EccState.SectorSize *
                               (hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8
                                    ? 8U
                                    : 16U); /* Sector size in bits */
...
static HAL_StatusTypeDef
HAL_NAND_ECC_Read_Sector(NAND_HandleTypeDef *hnand, uint32_t SectorOffset,
                         uint32_t EccOffset, void *Buff, uint32_t BuffOffset)
{
   HAL_StatusTypeDef status;
   ...
   /* The ECC is automatically computed by hardware, now try to correct errors
    * if any */
   if (hnand->Init.EccAlgorithm == FMC_NAND_ECC_ALGO_BCH) {
      /* For BCH, the read ECC and the computed ECC are automatically used to
       * compute error positions */
      status =
          FMC_NAND_ECC_GetBCHResult(hnand->Instance, BCHDSRx, hnand->TimeOut);
      if (status == HAL_OK) {
         FMC_NAND_ECC_DecodeBCHResult(BCHDSRx, errorPositions, &errorCount);
         HAL_NAND_ECC_CorrectBCH(hnand, Buff, BuffOffset, errorCount,
                                 errorPositions);
      } else {
         hnand->State     = HAL_NAND_STATE_ERROR;
         hnand->ErrorCode = HAL_NAND_ERROR_TIMEOUT;
      ...
      /* For Hamming, we must retrieve the computed ECC and find ourselves the
       * error position */
      status = FMC_NAND_GetECC(hnand->Instance, eccWhenReading,
                               hnand->Init.NandBank, hnand->TimeOut);
      if (status == HAL_OK) {
         errorCount = HAL_NAND_ECC_CorrectHamming(
             hnand, Buff, BuffOffset, hnand->EccState.EccLength,
...
static HAL_StatusTypeDef HAL_NAND_ECC_Read_SinglePage(NAND_HandleTypeDef *hnand,
                                                      uint32_t NandAddress,
                                                      void *Buff,
                                                      uint32_t BuffOffset)
{
   ...
   if (status == HAL_OK) {
      do {
         ...
         (void)FMC_NAND_ECC_Enable(hnand->Instance, hnand->Init.NandBank);
         /* Read sector */
         status = HAL_NAND_ECC_Read_Sector(hnand, sectorOffset, eccOffset, Buff,
                                           BuffOffset + sectorOffset);
 ...
 * If no error occurred, HAL_OK is returned and NumPagesRead is equal to
 * NumPagesToRead. If read errors cannot be corrected with ECC, HAL_OK is
 * returned and NumPagesRead indicates the index of the concerned page. If a
 * non-ECC error occurred, a value different of HAL_OK is returned and
 * NumPagesRead indicates the index of the concerned page.
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 * @param  pAddress  pointer to NAND address structure
 * @param  pBuffer  pointer to destination read buffer
 ...
 * @retval HAL status
 */
static HAL_StatusTypeDef HAL_NAND_ECC_Read_Page(NAND_HandleTypeDef *hnand,
                                                NAND_AddressTypeDef *pAddress,
                                                void *pBuffer,
                                                uint32_t NumPagesToRead,
                                                uint32_t *NumPagesRead)
{
   ...
   if (hnand->State == HAL_NAND_STATE_READY) {
      ...
      if (hnand->Init.EccAlgorithm == FMC_NAND_ECC_ALGO_BCH) {
         (void)FMC_NAND_ECC_SetAccessMode(hnand->Instance,
      ...
      while ((numPagesRead < NumPagesToRead) &&
             (nandAddress < nandAddressMax)) {
         status = HAL_NAND_ECC_Read_SinglePage(hnand, nandAddress, pBuffer,
                                               bufferOffset);
         if (status != HAL_OK) {
            hnand->State = HAL_NAND_STATE_ERROR;
            break;
...
/**
 * @brief  Read Page(s) from NAND memory block with error correction (8-bit
 * addressing) If no error occurred, HAL_OK is returned and NumPagesRead is
 * equal to NumPagesToRead. If read errors cannot be corrected with ECC, HAL_OK
 * is returned and NumPagesRead indicates the index of the concerned page. If a
 * non-ECC error occurred, a value different of HAL_OK is returned and
 * NumPagesRead indicates the index of the concerned page.
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_ECC_Read_Page_8b(NAND_HandleTypeDef *hnand,
                                            NAND_AddressTypeDef *pAddress,
                                            uint8_t *pBuffer,
                                            uint32_t NumPagesToRead,
                                            uint32_t *NumPagesRead)
{
   assert(hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8);
   return HAL_NAND_ECC_Read_Page(hnand, pAddress, pBuffer, NumPagesToRead,
                                 NumPagesRead);
}

/**
 * @brief  Read Page(s) from NAND memory block with error correction (16-bit
 * addressing) If no error occurred, HAL_OK is returned and NumPagesRead is
 * equal to NumPagesToRead. If read errors cannot be corrected with ECC, HAL_OK
 * is returned and NumPagesRead indicates the index of the concerned page. If a
 * non-ECC error occurred, a value different of HAL_OK is returned and
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_ECC_Read_Page_16b(NAND_HandleTypeDef *hnand,
                                             NAND_AddressTypeDef *pAddress,
                                             uint16_t *pBuffer,
                                             uint32_t NumPagesToRead,
                                             uint32_t *NumPagesRead)
...
static HAL_StatusTypeDef
HAL_NAND_ECC_Write_Sector(NAND_HandleTypeDef *hnand, uint32_t SectorOffset,
                          uint32_t EccOffset, void *Buff, uint32_t BuffOffset)
{
   HAL_StatusTypeDef status;
   ...
   if (hnand->Init.EccAlgorithm == FMC_NAND_ECC_ALGO_BCH) {
      status =
          FMC_NAND_ECC_GetBCH(hnand->Instance, EccWhenWriting, hnand->TimeOut);
   } else {
      status = FMC_NAND_GetECC(hnand->Instance, EccWhenWriting,
                               hnand->Init.NandBank, hnand->TimeOut);
   }
   if (status == HAL_OK) {
...
static HAL_StatusTypeDef
HAL_NAND_ECC_Write_SinglePage(NAND_HandleTypeDef *hnand, uint32_t NandAddress,
                              void *Buff, uint32_t BuffOffset)
{
   ...
   do {
      ...
      (void)FMC_NAND_ECC_Enable(hnand->Instance, hnand->Init.NandBank);
      /* Write sector */
      status = HAL_NAND_ECC_Write_Sector(hnand, sectorOffset, eccOffset, Buff,
                                         BuffOffset + sectorOffset);
      /* Reset ECC computations */
      (void)FMC_NAND_ECC_Disable(hnand->Instance, hnand->Init.NandBank);
      if (status != HAL_OK) {
 ...
 * If no error occurred, HAL_OK is returned and NumPagesWritten is equal to
 * NumPagesToWrite. If an error occurred, a value different of HAL_OK is
 * returned and NumPagesWritten indicates the index of the concerned page.
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 * @param  pAddress  pointer to NAND address structure
 * @param  pBuffer  pointer to source buffer to write
 ...
 * @retval HAL status
 */
static HAL_StatusTypeDef HAL_NAND_ECC_Write_Page(NAND_HandleTypeDef *hnand,
                                                 NAND_AddressTypeDef *pAddress,
                                                 void *pBuffer,
                                                 uint32_t NumPagesToWrite,
                                                 uint32_t *NumPagesWritten)
{
   HAL_StatusTypeDef status = HAL_OK;
   uint32_t nandAddressMax;
   uint32_t nandAddress;
   ...
   if (hnand->State == HAL_NAND_STATE_READY) {
      ...
      while ((numPagesWritten < NumPagesToWrite) &&
             (nandAddress < nandAddressMax)) {
         status = HAL_NAND_ECC_Write_SinglePage(hnand, nandAddress, pBuffer,
                                                bufferOffset);
         if (status == HAL_ERROR) {
            hnand->State = HAL_NAND_STATE_READY;
            break;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_ECC_Write_Page_8b(NAND_HandleTypeDef *hnand,
                                             NAND_AddressTypeDef *pAddress,
                                             uint8_t *pBuffer,
                                             uint32_t NumPagesToWrite,
                                             uint32_t *NumPagesWritten)
{
   assert(hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8);
   return HAL_NAND_ECC_Write_Page(hnand, pAddress, pBuffer, NumPagesToWrite,
                                  NumPagesWritten);
}
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_ECC_Write_Page_16b(NAND_HandleTypeDef *hnand,
                                              NAND_AddressTypeDef *pAddress,
                                              uint16_t *pBuffer,
                                              uint32_t NumPagesToWrite,
                                              uint32_t *NumPagesWritten)
 ...
 */
void HAL_NAND_ECC_GetStatistics(NAND_HandleTypeDef *hnand,
                                NAND_EccStatisticsTypeDef *Stat)
{
...
/** @defgroup NAND_Exported_Functions_Group6 Input and Output functions with
error correction
  *  @brief   Peripheral State functions
  *
@verbatim
...
/**
 * @brief  Compute the number of 32-bit words needed by the sequencer to store
 * the ECC of a sector
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *               the configuration information for NAND module.
...
{
   return hnand->Init.EccAlgorithm == FMC_NAND_ECC_ALGO_HAMMING
              ? 1
              : 5; /* HECCR or BCHDSR0 to BCHDSR4 */
}

/**
 * @brief  Compute the size of the buffer needed by the sequencer to store the
 * ECC of a page
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 ...
 * This function calls the weak function HAL_NAND_Sequencer_MspInit() which
 * allows to setup the 3 DMA channels.
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 ...
 */
HAL_StatusTypeDef
HAL_NAND_Sequencer_Init(NAND_HandleTypeDef *hnand,
                        const NAND_SequencerConfigTypeDef *Config)
{
   hnand->SequencerConfig = *Config;
   hnand->SequencerState.EccBufferSize =
       HAL_NAND_Sequencer_ComputeEccBufferSize(hnand);
...
static void HAL_NAND_Sequencer_setup(NAND_HandleTypeDef *hnand,
                                     uint32_t NandAddress, bool WriteFlag,
                                     bool RawFlag)
{
   uint32_t csqcfr1;
   uint32_t csqcfr2;
   ...
   (void)FMC_NAND_ECC_SetAccessMode(hnand->Instance,
                                    WriteFlag ? FMC_NAND_ECC_ACCESS_MODE_WRITE
                                              : FMC_NAND_ECC_ACCESS_MODE_READ);
   ...
   if (WriteFlag) {
      ...
      csqcfr1 |= (NAND_CMD_AREA_A << FMC_CSQCFGR1_CMD1_Pos) |
                 FMC_CSQCFGR1_CMD2EN |
   ...
   if (WriteFlag) {
      ...
      csqcfr2 = (NAND_CMD_RANDOM_OUTPUT_0 << FMC_CSQCFGR2_RCMD1_Pos) |
   ...
   csqar2 = hnand->ChipSelect == NAND_NCE1_OFFSET ? FMC_CSQAR2_NANDCEN_0
                                                  : FMC_CSQAR2_NANDCEN_1;
   csqar2 |= (hnand->Config.PageSize + hnand->EccConfig.Offset)
             << FMC_CSQAR2_SAO_Pos;
   if (totalPageNumber > NAND_MAX_PAGES_FOR_16BIT_ADDRESS) {
   ...
   MODIFY_REG(hnand->Instance->CSQCFGR1,
              FMC_CSQCFGR1_CMD2EN_Msk | FMC_CSQCFGR1_DMADEN_Msk |
                  FMC_CSQCFGR1_ACYNBR_Msk | FMC_CSQCFGR1_CMD1_Msk |
                  FMC_CSQCFGR1_CMD2_Msk | FMC_CSQCFGR1_CMD1T_Msk |
                  FMC_CSQCFGR1_CMD2T_Msk,
              csqcfr1);
   MODIFY_REG(hnand->Instance->CSQCFGR2,
              FMC_CSQCFGR2_SQSDTEN_Msk | FMC_CSQCFGR2_RCMD2EN_Msk |
                  FMC_CSQCFGR2_DMASEN_Msk | FMC_CSQCFGR2_RCMD1_Msk |
                  FMC_CSQCFGR2_RCMD2_Msk | FMC_CSQCFGR2_RCMD1T_Msk |
                  FMC_CSQCFGR2_RCMD2T_Msk,
              csqcfr2);
   MODIFY_REG(hnand->Instance->CSQCFGR3,
              FMC_CSQCFGR3_SNBR_Msk | FMC_CSQCFGR3_AC1T_Msk |
                  FMC_CSQCFGR3_AC2T_Msk | FMC_CSQCFGR3_AC3T_Msk |
                  FMC_CSQCFGR3_AC4T_Msk | FMC_CSQCFGR3_AC5T_Msk |
                  FMC_CSQCFGR3_SDT_Msk | FMC_CSQCFGR3_RAC1T_Msk |
                  FMC_CSQCFGR3_RAC2T_Msk,
              csqcfr3);
   MODIFY_REG(hnand->Instance->CSQAR1,
              FMC_CSQAR1_ADDC1_Msk | FMC_CSQAR1_ADDC2_Msk |
                  FMC_CSQAR1_ADDC3_Msk | FMC_CSQAR1_ADDC4_Msk,
              csqar1);
   MODIFY_REG(hnand->Instance->CSQAR2,
              FMC_CSQAR2_ADDC5_Msk | FMC_CSQAR2_NANDCEN_Msk |
                  FMC_CSQAR2_SAO_Msk,
              csqar2);
}
...
HAL_StatusTypeDef HAL_NAND_Sequencer_Abort(NAND_HandleTypeDef *hnand)
{
   ...
   if (hnand->State == HAL_NAND_STATE_BUSY) {
      /* FIXME: No information has been found on how to abort the sequencer.
         But the word 'abort' is evoked in 22.9.8 and for "TCF: Transfer
         Complete flag". */

      if (HAL_MDMA_GetState(hnand->SequencerConfig.HdmaRead) ==
          HAL_MDMA_STATE_BUSY) {
      ...
      if (HAL_MDMA_GetState(hnand->SequencerConfig.HdmaWrite) ==
          HAL_MDMA_STATE_BUSY) {
      ...
      if (HAL_MDMA_GetState(hnand->SequencerConfig.HdmaReadEcc) ==
          HAL_MDMA_STATE_BUSY) {
...
static HAL_StatusTypeDef
HAL_NAND_Sequencer_WaitTransferCompletion(NAND_HandleTypeDef *hnand,
                                          uint32_t TimeOut)
{
   HAL_StatusTypeDef status = HAL_OK;
   uint32_t tickstart;
   ...
   while (!READ_BIT(hnand->Instance->CSQISR, FMC_CSQISR_TCF)) {
      if ((TimeOut != HAL_MAX_DELAY) &&
         ...
         /* If timeout then abort the current transfer. No need to check return
            value: as in this case we will return HAL_ERROR with
            HAL_MDMA_ERROR_TIMEOUT error code */
...
static uint32_t HAL_NAND_ECC_ReadHamming(uint32_t EccSize,
...
static void HAL_NAND_Sequencer_Correct(NAND_HandleTypeDef *hnand)
{
   ...
   if (sectorMap) {
      ...
      /* Change read position (column) to ECC data */
      HAL_NAND_ChangeReadColumn(hnand, hnand->Config.PageSize +
                                           hnand->EccConfig.Offset);
      ...
      for (sectorIndex = 0; sectorIndex < sectorCount; sectorIndex++) {
         if (sectorMap & (1 << sectorIndex)) {
            ...
            if (hnand->Init.EccAlgorithm == FMC_NAND_ECC_ALGO_HAMMING) {
               uint32_t eccWhenWriting = HAL_NAND_ECC_ReadHamming(
                   eccSize, &eccWhenWritingBuffer[sectorIndex * eccSize]);
               uint32_t eccWhenReading =
                   hnand->SequencerConfig.EccBuffer[sectorIndex];
               errorCount = HAL_NAND_ECC_CorrectHamming(
                   hnand, hnand->SequencerState.Data, dataOffset,
                   hnand->EccState.EccLength, eccWhenWriting, eccWhenReading);
            } else {
               uint32_t *BCHDSRx =
                   &hnand->SequencerConfig.EccBuffer[sectorIndex * 5];
               uint16_t errorPositions[8];
               FMC_NAND_ECC_DecodeBCHResult(BCHDSRx, errorPositions,
                                            &errorCount);
               HAL_NAND_ECC_CorrectBCH(hnand, hnand->SequencerState.Data,
                                       dataOffset, errorCount, errorPositions);
            }
            HAL_NAND_ECC_UpdateErrorCounters(hnand, errorCount);
         }
      }
...
HAL_StatusTypeDef HAL_NAND_Sequencer_WaitCompletion(NAND_HandleTypeDef *hnand,
                                                    uint32_t TimeOut)
{
   HAL_StatusTypeDef status;
   ...
   if (hnand->State == HAL_NAND_STATE_BUSY) {
      ...
      status = HAL_MDMA_PollForTransfer(hnand->SequencerConfig.HdmaRead,
                                        HAL_MDMA_FULL_TRANSFER, TimeOut);
      if (status != HAL_OK && hnand->SequencerConfig.HdmaRead->ErrorCode !=
      ...
      status = HAL_MDMA_PollForTransfer(hnand->SequencerConfig.HdmaWrite,
                                        HAL_MDMA_FULL_TRANSFER, TimeOut);
      if (status != HAL_OK && hnand->SequencerConfig.HdmaWrite->ErrorCode !=
                                  HAL_MDMA_ERROR_NO_XFER) {
         hnand->State     = HAL_NAND_STATE_ERROR;
         hnand->ErrorCode = HAL_NAND_ERROR_TIMEOUT;
         __HAL_UNLOCK(hnand);
      ...
      status = HAL_MDMA_PollForTransfer(hnand->SequencerConfig.HdmaReadEcc,
                                        HAL_MDMA_FULL_TRANSFER, TimeOut);
      if (status != HAL_OK && hnand->SequencerConfig.HdmaReadEcc->ErrorCode !=
                                  HAL_MDMA_ERROR_NO_XFER) {
         hnand->State     = HAL_NAND_STATE_ERROR;
         hnand->ErrorCode = HAL_NAND_ERROR_TIMEOUT;
         __HAL_UNLOCK(hnand);
...
static HAL_StatusTypeDef
HAL_NAND_Sequencer_startTransfer(NAND_HandleTypeDef *hnand, void *Buffer,
                                 bool WriteFlag, bool RawFlag)
{
   HAL_StatusTypeDef status;
   ...
   size   = HAL_NAND_ComputeWordsToBytes(hnand->EccState.SectorSize,
   ...
   if (!WriteFlag && !RawFlag) {
      ...
      srcAddress = hnand->Init.EccAlgorithm == FMC_NAND_ECC_ALGO_HAMMING
                       ? (uint32_t)&hnand->Instance->HECCR
   ...
   WRITE_REG(hnand->Instance->CSQICR, FMC_CSQICR_CTCF | FMC_CSQICR_CSCF |
                                          FMC_CSQICR_CSEF | FMC_CSQICR_CSUEF |
                                          FMC_CSQICR_CCMDTCF);
   /* Start the sequencer. */
...
static HAL_StatusTypeDef
HAL_NAND_Sequencer_transferPage(NAND_HandleTypeDef *hnand,
                                NAND_AddressTypeDef *pAddress, void *pBuffer,
                                bool WriteFlag, bool RawFlag)
{
   HAL_StatusTypeDef status = HAL_OK;
   ...
   if (hnand->State == HAL_NAND_STATE_READY) {
      ...
      if (nandAddress < nandAddressMax) {
         ...
         status = HAL_NAND_Sequencer_startTransfer(hnand, pBuffer, WriteFlag,
                                                   RawFlag);
         if (status != HAL_OK) {
            __HAL_UNLOCK(hnand);
         }
...
/**
 * @brief  Read Page(s) from NAND memory block with error correction using the
 * sequencer (8-bit addressing) If no error occurred, HAL_OK is returned. If an
 * error occurred, a value different of HAL_OK is returned.
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 * @param  pAddress  pointer to NAND address structure
 * @param  pBuffer  pointer to destination read buffer
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Sequencer_Read_Page_8b(NAND_HandleTypeDef *hnand,
                                                  NAND_AddressTypeDef *pAddress,
                                                  uint8_t *pBuffer)
{
   assert(hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8);
   return HAL_NAND_Sequencer_transferPage(hnand, pAddress, pBuffer, false,
                                          true);
}

/**
 * @brief  Read Page(s) from NAND memory block with error correction using the
 * sequencer (16-bit addressing) If no error occurred, HAL_OK is returned. If an
 * error occurred, a value different of HAL_OK is returned.
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 * @param  pAddress  pointer to NAND address structure
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Sequencer_Read_Page_16b(
    NAND_HandleTypeDef *hnand, NAND_AddressTypeDef *pAddress, uint16_t *pBuffer)
{
   assert(hnand->Init.MemoryDataWidth != NAND_BUS_WIDTH_8);
   return HAL_NAND_Sequencer_transferPage(hnand, pAddress, pBuffer, false,
...
/**
 * @brief  Read Page(s) from NAND memory block with error correction using the
 * sequencer (8-bit addressing) If no error occurred, HAL_OK is returned. If
 * read errors cannot be corrected with ECC, HAL_OK is returned and
 * hnand->EccState.ErrorCount gives the number of error found. If a non-ECC
 * error occurred, a value different of HAL_OK is returned.
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 * @param  pAddress  pointer to NAND address structure
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Sequencer_ECC_Read_Page_8b(
    NAND_HandleTypeDef *hnand, NAND_AddressTypeDef *pAddress, uint8_t *pBuffer)
{
   assert(hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8);
   return HAL_NAND_Sequencer_transferPage(hnand, pAddress, pBuffer, false,
...
/**
 * @brief  Read one page from NAND memory with error correction using the
 * sequencer (16-bit addressing) If no error occurred, HAL_OK is returned. If
 * read errors cannot be corrected with ECC, HAL_OK is returned and
 * hnand->EccState.ErrorCount gives the number of error found. If a non-ECC
 * error occurred, a value different of HAL_OK is returned.
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Sequencer_ECC_Read_Page_16b(
    NAND_HandleTypeDef *hnand, NAND_AddressTypeDef *pAddress, uint16_t *pBuffer)
{
   assert(hnand->Init.MemoryDataWidth != NAND_BUS_WIDTH_8);
   return HAL_NAND_Sequencer_transferPage(hnand, pAddress, pBuffer, false,
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Sequencer_Write_Page_8b(
    NAND_HandleTypeDef *hnand, NAND_AddressTypeDef *pAddress, uint8_t *pBuffer)
{
   assert(hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8);
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Sequencer_Write_Page_16b(
    NAND_HandleTypeDef *hnand, NAND_AddressTypeDef *pAddress, uint16_t *pBuffer)
{
   assert(hnand->Init.MemoryDataWidth != NAND_BUS_WIDTH_8);
   return HAL_NAND_Sequencer_transferPage(hnand, pAddress, pBuffer, true, true);
...
/**
 * @brief  Write one page to NAND memory with error correction using the
 * sequencer If no error occurred, HAL_OK is returned. If an error occurred, a
 * value different of HAL_OK is returned.
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 * @param  pAddress  pointer to NAND address structure
 * @param  pBuffer  pointer to source buffer to write
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Sequencer_ECC_Write_Page_8b(
    NAND_HandleTypeDef *hnand, NAND_AddressTypeDef *pAddress, uint8_t *pBuffer)
{
   assert(hnand->Init.MemoryDataWidth == NAND_BUS_WIDTH_8);
   return HAL_NAND_Sequencer_transferPage(hnand, pAddress, pBuffer, true,
                                          false);
}

/**
 * @brief  Write one page to NAND memory with error correction using the
 * sequencer If no communication error occurred, HAL_OK is returned and
 * hnand->EccState.ErrorCount gives the number of error found. If an error
 * occurred, a value different of HAL_OK is returned.
 * @param  hnand pointer to a NAND_HandleTypeDef structure that contains
 *                the configuration information for NAND module.
 * @param  pAddress  pointer to NAND address structure
 * @param  pBuffer  pointer to source buffer to write
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_NAND_Sequencer_ECC_Write_Page_16b(
    NAND_HandleTypeDef *hnand, NAND_AddressTypeDef *pAddress, uint16_t *pBuffer)
{
   assert(hnand->Init.MemoryDataWidth != NAND_BUS_WIDTH_8);
   return HAL_NAND_Sequencer_transferPage(hnand, pAddress, pBuffer, true,
