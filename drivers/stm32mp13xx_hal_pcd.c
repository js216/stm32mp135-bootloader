                                           ...
                                           pPCD_CallbackTypeDef pCallback)
{
   ...
   if (hpcd->State == HAL_PCD_STATE_READY) {
      switch (CallbackID) {
         case HAL_PCD_SOF_CB_ID: hpcd->SOFCallback = pCallback; break;

         case HAL_PCD_SETUPSTAGE_CB_ID:
            hpcd->SetupStageCallback = pCallback;
            break;

         case HAL_PCD_RESET_CB_ID: hpcd->ResetCallback = pCallback; break;

         case HAL_PCD_SUSPEND_CB_ID: hpcd->SuspendCallback = pCallback; break;

         case HAL_PCD_RESUME_CB_ID: hpcd->ResumeCallback = pCallback; break;

         case HAL_PCD_CONNECT_CB_ID: hpcd->ConnectCallback = pCallback; break;

         case HAL_PCD_DISCONNECT_CB_ID:
            hpcd->DisconnectCallback = pCallback;
            break;

         case HAL_PCD_MSPINIT_CB_ID: hpcd->MspInitCallback = pCallback; break;
                           ...
                           HAL_PCD_CallbackIDTypeDef CallbackID)
{
   ...
   if (hpcd->State == HAL_PCD_STATE_READY) {
      switch (CallbackID) {
         case HAL_PCD_SOF_CB_ID: hpcd->SOFCallback = HAL_PCD_SOFCallback; break;
   ...
   if (USB_GetMode(hpcd->Instance) == USB_OTG_MODE_DEVICE) {
      ...
      if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_OEPINT)) {
         ...
         while (ep_intr != 0U) {
            if ((ep_intr & 0x1U) != 0U) {
               epint =
                   USB_ReadDevOutEPInterrupt(hpcd->Instance, (uint8_t)epnum);
               ...
               if ((epint & USB_OTG_DOEPINT_EPDISD) == USB_OTG_DOEPINT_EPDISD) {
                  if ((USBx->GINTSTS & USB_OTG_GINTSTS_GONAKEFF) ==
                      USB_OTG_GINTSTS_GONAKEFF) {
      ...
      if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_ENUMDNE)) {
         ...
         (void)USB_SetTurnaroundTime(hpcd->Instance, HAL_RCC_GetHCLK2Freq(),
                                      ...
                                      uint8_t testmode)
{
   ...
   switch (testmode) {
      ...
      case TEST_SE0_NAK:
      case TEST_PACKET:
      case TEST_FORCE_EN: USBx_DEVICE->DCTL |= (uint32_t)testmode << 4; break;
