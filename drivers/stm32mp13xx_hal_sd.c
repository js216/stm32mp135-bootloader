/**
  ...
  ******************************************************************************
  ...
  performed by the user before calling HAL_SD_Init. Basically, the
      ...
      switch (CallbackID) {
         ...
         case HAL_SD_WRITE_DMA_LNKLST_BUF_CPLT_CB_ID:
            hsd->Write_DMALnkLstBufCpltCallback =
                HAL_SDEx_Write_DMALnkLstBufCpltCallback;
            break;
         case HAL_SD_MSP_INIT_CB_ID: break;
         case HAL_SD_MSP_DEINIT_CB_ID:
            hsd->MspDeInitCallback = HAL_SD_MspDeInit;
            break;
         default:
                                       ...
                                       HAL_SD_CardStatusTypeDef *pStatus)
{
   ...
   if (errorstate != HAL_SD_ERROR_NONE) {
      ...
      pStatus->AllocationUnitSize =
          (uint8_t)((sd_status[2] & 0xF00000U) >> 20U);
      ...
      if (sdmmc_clk != 0U) {
         ...
         if (hsd->Init.ClockDiv >= (sdmmc_clk / (2U * SD_NORMAL_SPEED_FREQ))) {
            ...
            if (hsd->Init.ClockDiv == 0U) {
               ...
               if ((sdmmc_clk / (2U * hsd->Init.ClockDiv)) >
                   SD_HIGH_SPEED_FREQ) {
                  Init.ClockDiv = sdmmc_clk / (2U * SD_HIGH_SPEED_FREQ);
               } else {
                  Init.ClockDiv = hsd->Init.ClockDiv;
...
HAL_StatusTypeDef HAL_SD_Abort(SD_HandleTypeDef *hsd)
{
   ...
   if (hsd->State == HAL_SD_STATE_BUSY) {
      ...
      if ((hsd->Instance->DCTRL & SDMMC_DCTRL_DTDIR) ==
         ...
         if (hsd->ErrorCode == HAL_SD_ERROR_NONE) {
            while (!__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_DABORT |
         ...
         while (
             !__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_DABORT | SDMMC_FLAG_DATAEND)) {
            if ((HAL_GetTick() - tickstart) >= SDMMC_DATATIMEOUT) {
               hsd->ErrorCode = HAL_SD_ERROR_TIMEOUT;
               hsd->State     = HAL_SD_STATE_READY;
               return HAL_TIMEOUT;
      ...
      /*The reason of all these while conditions previously is that we need to
        wait the SDMMC and clear the appropriate flags that will be set
        depending of the abort/non abort of the memory */
      /*Not waiting the SDMMC flags will cause the next SDMMC_DISABLE_IDMA to
        not get cleared and will result in next SDMMC read/write operation to
        fail */
...
static uint32_t SD_WideBus_Enable(SD_HandleTypeDef *hsd)
{
   ...
   if ((scr[1U] & SDMMC_WIDE_BUS_SUPPORT) != SDMMC_ALLZERO) {
      ...
      errorstate = SDMMC_CmdAppCommand(
          hsd->Instance, (uint32_t)(hsd->SdCard.RelCardAdd << 16U));
      if (errorstate != HAL_SD_ERROR_NONE) {
         return errorstate;
      }
                                  ...
                                  uint32_t UltraHighSpeedMode)
{
   ...
   if (hsd->SdCard.CardSpeed == CARD_ULTRA_HIGH_SPEED) {
    ...
    (USE_HAL_SD_REGISTER_CALLBACKS == 1U)
         ...
         /* SDMMC_FB_CLK tuned feedback clock selected as receive clock, for
          * SDR104 */
...
__weak void HAL_SDEx_Read_DMALnkLstBufCpltCallback(SD_HandleTypeDef *hsd)
{
   ...
   /* NOTE : This function should not be modified, when the callback is needed,
             the HAL_SDEx_Read_DMALnkLstBufCpltCallback can be implemented in
      the user file
    */
}
