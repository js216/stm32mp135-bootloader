...
HAL_StatusTypeDef HAL_ETH_Init(ETH_HandleTypeDef *heth)
{
   ...
   WRITE_REG(heth->Instance->MAC1USTCR,
             (((uint32_t)HAL_RCC_GetACLKFreq() / ETH_MAC_US_TICK) - 1U));
   ...
   if ((heth->Init.RxBuffLen % 0x4U) != 0x0U) {
      ...
      MODIFY_REG(heth->Instance->DMAC0RXCR, ETH_DMAC0RXCR_RBSZ,
   ...
   SET_BIT(heth->Instance->MMCRXIMR,
           ETH_MMCRXIMR_RXLPITRCIM | ETH_MMCRXIMR_RXLPIUSCIM |
               ETH_MMCRXIMR_RXUCGPIM | ETH_MMCRXIMR_RXALGNERPIM |
               ETH_MMCRXIMR_RXCRCERPIM);

   /* Disable Tx MMC Interrupts */
   SET_BIT(heth->Instance->MMCTXIMR,
           ETH_MMCTXIMR_TXLPITRCIM | ETH_MMCTXIMR_TXLPIUSCIM |
               ETH_MMCTXIMR_TXGPKTIM | ETH_MMCTXIMR_TXMCOLGPIM |
               ETH_MMCTXIMR_TXSCOLGPIM);
                                           ...
                                           pETH_CallbackTypeDef pCallback)
{
   ...
   if (heth->gState == HAL_ETH_STATE_READY) {
      switch (CallbackID) {
         ...
         case HAL_ETH_ERROR_CB_ID: heth->ErrorCallback = pCallback; break;

         case HAL_ETH_PMT_CB_ID: heth->PMTCallback = pCallback; break;

         case HAL_ETH_EEE_CB_ID: heth->EEECallback = pCallback; break;

         case HAL_ETH_WAKEUP_CB_ID: heth->WakeUpCallback = pCallback; break;

         case HAL_ETH_MSPINIT_CB_ID: heth->MspInitCallback = pCallback; break;
                           ...
                           HAL_ETH_CallbackIDTypeDef CallbackID)
{
   ...
   if (heth->gState == HAL_ETH_STATE_READY) {
      switch (CallbackID) {
         ...
         case HAL_ETH_PMT_CB_ID: heth->PMTCallback = HAL_ETH_PMTCallback; break;

         case HAL_ETH_EEE_CB_ID: heth->EEECallback = HAL_ETH_EEECallback; break;
...
HAL_StatusTypeDef HAL_ETH_Start_IT(ETH_HandleTypeDef *heth)
{
   if (heth->gState == HAL_ETH_STATE_READY) {
      ...
      - Fatal bus interrupt
      */
      __HAL_ETH_DMA_ENABLE_IT(heth, (ETH_DMAC0IER_NIE | ETH_DMAC0IER_RIE |
                                     ETH_DMAC0IER_TIE | ETH_DMAC0IER_FBEE |
                                     ETH_DMAC0IER_AIE | ETH_DMAC0IER_RBUE));
...
HAL_StatusTypeDef HAL_ETH_Stop_IT(ETH_HandleTypeDef *heth)
{
   ...
   if (heth->gState == HAL_ETH_STATE_STARTED) {
      ...
      - Fatal bus interrupt
      */
      __HAL_ETH_DMA_DISABLE_IT(heth, (ETH_DMAC0IER_NIE | ETH_DMAC0IER_RIE |
                                      ETH_DMAC0IER_TIE | ETH_DMAC0IER_FBEE |
                                      ETH_DMAC0IER_AIE | ETH_DMAC0IER_RBUE));
                                   ...
                                   uint32_t Timeout)
{
   ...
   if (heth->gState == HAL_ETH_STATE_STARTED) {
      ...
      if (ETH_Prepare_Tx_Descriptors(heth, pTxConfig, 0) !=
          HAL_ETH_ERROR_NONE) {
      ...
      /* Start transmission */
      /* issue a poll command to Tx DMA by writing address of next immediate
       * free descriptor */
      WRITE_REG(
          heth->Instance->DMAC0TXDTPR,
                                      ...
                                      ETH_TxPacketConfigTypeDef *pTxConfig)
{
   ...
   if (heth->gState == HAL_ETH_STATE_STARTED) {
      ...
      /* Config DMA Tx descriptor by Tx Packet info */
      if (ETH_Prepare_Tx_Descriptors(heth, pTxConfig, 1) !=
          HAL_ETH_ERROR_NONE) {
         heth->ErrorCode |= HAL_ETH_ERROR_BUSY;
         return HAL_ERROR;
      }
...
HAL_StatusTypeDef HAL_ETH_ReadData(ETH_HandleTypeDef *heth, void **pAppBuff)
{
   ...
   while (
      ...
      if ((READ_BIT(dmarxdesc->DESC3, ETH_DMARXNDESCWBF_FD) !=
           (uint32_t)RESET) ||
         ...
         heth->rxLinkCallback(heth, &heth->RxDescList.pRxStart,
                              &heth->RxDescList.pRxEnd,
                              (uint8_t *)dmarxdesc->BackupAddr0, bufflength);
#else
         /* Link callback */
         HAL_ETH_RxLinkCallback(
             heth, &heth->RxDescList.pRxStart, &heth->RxDescList.pRxEnd,
...
static void ETH_UpdateDescriptor(ETH_HandleTypeDef *heth)
{
   ...
   if (heth->RxDescList.RxBuildDescCnt != desccount) {
      ...
      WRITE_REG(heth->Instance->DMAC0RXDTPR,
 ...
 * @retval None
 */
__weak void HAL_ETH_RxLinkCallback(ETH_HandleTypeDef *heth, void **pStart,
      ...
      if (pktInUse != 0U) {
         ...
         if ((heth->Init.TxDesc[idx].DESC3 & ETH_DMATXNDESCRF_OWN) == 0U) {
            ...
            if (timestamp->TimeStampHigh != UINT32_MAX &&
               ...
               heth->txPtpCallback(heth, dmatxdesclist->PacketAddress[idx],
                                   timestamp);
            }
#endif /* HAL_ETH_USE_PTP */
            /* Release the packet.  */
            ...
            if (timestamp->TimeStampHigh != UINT32_MAX &&
                timestamp->TimeStampLow != UINT32_MAX) {
               HAL_ETH_TxPtpCallback(heth, dmatxdesclist->PacketAddress[idx],
                                     timestamp);
            }
#endif /* HAL_ETH_USE_PTP */
            /* Release the packet.  */
 ...
 * @retval None
 */
__weak void HAL_ETH_TxPtpCallback(ETH_HandleTypeDef *heth, uint32_t *buff,
                                  ETH_TimeStampTypeDef *timestamp)
{
   /* Prevent unused argument(s) compilation warning */
   UNUSED(buff);
...
void HAL_ETH_IRQHandler(ETH_HandleTypeDef *heth)
{
   ...
   /* Packet received */
   if (((dma_flag & ETH_DMAC0SR_RI) != 0U) &&
       ((dma_itsource & ETH_DMAC0IER_RIE) != 0U)) {
   ...
   /* Packet transmitted */
   if (((dma_flag & ETH_DMAC0SR_TI) != 0U) &&
       ((dma_itsource & ETH_DMAC0IER_TIE) != 0U)) {
   ...
   /* ETH DMA Error */
   if (((dma_flag & ETH_DMAC0SR_AIS) != 0U) &&
       ((dma_itsource & ETH_DMAC0IER_AIE) != 0U)) {
      heth->ErrorCode |= HAL_ETH_ERROR_DMA;
      /* if fatal bus error occurred */
      if ((dma_flag & ETH_DMAC0SR_FBE) != 0U) {
         /* Get DMA error code  */
         heth->DMAErrorCode =
             READ_BIT(heth->Instance->DMAC0SR,
                      (ETH_DMAC0SR_FBE | ETH_DMAC0SR_TPS | ETH_DMAC0SR_RPS));
         ...
         heth->DMAErrorCode =
             READ_BIT(heth->Instance->DMAC0SR,
                      (ETH_DMAC0SR_CDE | ETH_DMAC0SR_ETI | ETH_DMAC0SR_RWT |
                       ETH_DMAC0SR_RBU | ETH_DMAC0SR_AIS));

         /* Clear the interrupt summary flag */
         __HAL_ETH_DMA_CLEAR_IT(heth, (ETH_DMAC0SR_CDE | ETH_DMAC0SR_ETI |
                                       ETH_DMAC0SR_RWT | ETH_DMAC0SR_RBU |
                                       ETH_DMAC0SR_AIS));
      }
#if (USE_HAL_ETH_REGISTER_CALLBACKS == 1)
   ...
   if ((mac_flag & ETH_MAC_PMT_IT) != 0U) {
      ...
      heth->MACWakeUpEvent =
          READ_BIT(heth->Instance->MACPCSR,
                                          ...
                                          uint32_t *pRegValue)
{
   ...
   if (READ_BIT(heth->Instance->MACMDIOAR, ETH_MACMDIOAR_GB) !=
       (uint32_t)RESET) {
      return HAL_ERROR;
   }
   ...
   MODIFY_REG(tmpreg, ETH_MACMDIOAR_GOC,
                                       ...
                                       ETH_MACConfigTypeDef *macconf)
{
   ...
   macconf->PortSelect =
       ((READ_BIT(heth->Instance->MACCR, ETH_MACCR_PS) >> 15) > 0U) ? ENABLE
                                                                    : DISABLE;
   macconf->JumboPacket =
   ...
   macconf->TransmitFlowControl =
       ((READ_BIT(heth->Instance->MACQ0TXFCR, ETH_MACQ0TXFCR_TFE) >> 1) > 0U)
           ? ENABLE
           : DISABLE;
   macconf->ZeroQuantaPause =
       ((READ_BIT(heth->Instance->MACQ0TXFCR, ETH_MACQ0TXFCR_DZPQ) >> 7) == 0U)
           ? ENABLE
           : DISABLE;
   macconf->PauseLowThreshold =
       READ_BIT(heth->Instance->MACQ0TXFCR, ETH_MACQ0TXFCR_PLT);
   macconf->PauseTime =
       (READ_BIT(heth->Instance->MACQ0TXFCR, ETH_MACQ0TXFCR_PT) >> 16);
   macconf->ReceiveFlowControl =
       (READ_BIT(heth->Instance->MACRXFCR, ETH_MACRXFCR_RFE) > 0U) ? ENABLE
                                                                   : DISABLE;
   macconf->UnicastPausePacketDetect =
       ((READ_BIT(heth->Instance->MACRXFCR, ETH_MACRXFCR_UP) >> 1) > 0U)
           ? ENABLE
           : DISABLE;

   macconf->TransmitQueueMode = READ_BIT(
       heth->Instance->MTLTXQ0OMR, (ETH_MTLTXQ0OMR_TTC | ETH_MTLTXQ0OMR_TSF));

   macconf->ReceiveQueueMode = READ_BIT(
       heth->Instance->MTLRXQ0OMR, (ETH_MTLRXQ0OMR_RTC | ETH_MTLRXQ0OMR_RSF));
   macconf->ForwardRxUndersizedGoodPacket =
       ((READ_BIT(heth->Instance->MTLRXQ0OMR, ETH_MTLRXQ0OMR_FUP) >> 3) > 0U)
           ? ENABLE
           : DISABLE;
   macconf->ForwardRxErrorPacket =
       ((READ_BIT(heth->Instance->MTLRXQ0OMR, ETH_MTLRXQ0OMR_FEP) >> 4) > 0U)
           ? ENABLE
           : DISABLE;
   macconf->DropTCPIPChecksumErrorPacket =
       ((READ_BIT(heth->Instance->MTLRXQ0OMR, ETH_MTLRXQ0OMR_DIS_TCP_EF) >>
         6) == 0U)
           ? ENABLE
           : DISABLE;
                                       ...
                                       ETH_DMAConfigTypeDef *dmaconf)
{
   ...
   dmaconf->RxOSRLimit =
       READ_BIT(heth->Instance->DMASBMR, ETH_DMASBMR_RD_OSR_LMT);
   dmaconf->TxOSRLimit =
       READ_BIT(heth->Instance->DMASBMR, ETH_DMASBMR_WR_OSR_LMT);

   dmaconf->DMAArbitration =
       READ_BIT(heth->Instance->DMAMR, (ETH_DMAMR_TXPR | ETH_DMAMR_TAA));

   dmaconf->PBLx8Mode =
       ((READ_BIT(heth->Instance->DMAC0CR, ETH_DMAC0CR_PBLX8) >> 16) > 0U)
           ? ENABLE
           : DISABLE;
   dmaconf->MaximumSegmentSize =
       READ_BIT(heth->Instance->DMAC0CR, ETH_DMAC0CR_MSS);

   dmaconf->FlushRxPacket =
       ((READ_BIT(heth->Instance->DMAC0RXCR, ETH_DMAC0RXCR_RPF) >> 31) > 0U)
           ? ENABLE
           : DISABLE;
   dmaconf->RxDMABurstLength =
       READ_BIT(heth->Instance->DMAC0RXCR, ETH_DMAC0RXCR_RXPBL);

   dmaconf->SecondPacketOperate =
       ((READ_BIT(heth->Instance->DMAC0TXCR, ETH_DMAC0TXCR_OSF) >> 4) > 0U)
           ? ENABLE
           : DISABLE;
   dmaconf->TCPSegmentation =
       ((READ_BIT(heth->Instance->DMAC0TXCR, ETH_DMAC0TXCR_TSE) >> 12) > 0U)
           ? ENABLE
           : DISABLE;
   dmaconf->TxDMABurstLength =
       READ_BIT(heth->Instance->DMAC0TXCR, ETH_DMAC0TXCR_TXPBL);
   dmaconf->AXIBLENMaxSize =
       READ_BIT(heth->Instance->DMASBMR,
                ETH_DMASBMR_BLEN256 | ETH_DMASBMR_BLEN128 | ETH_DMASBMR_BLEN64 |
                    ETH_DMASBMR_BLEN32 | ETH_DMASBMR_BLEN16 |
                    ETH_DMASBMR_BLEN8 | ETH_DMASBMR_BLEN4);
                           ...
                           const ETH_MACFilterConfigTypeDef *pFilterConfig)
{
   ...
   filterconfig =
       ((uint32_t)pFilterConfig->PromiscuousMode |
        ...
        ((uint32_t)((pFilterConfig->BroadcastFilter == ENABLE) ? 1U : 0U)
         << 5) |
...
void HAL_ETH_EnterPowerDownMode(
    ETH_HandleTypeDef *heth, const ETH_PowerDownConfigTypeDef *pPowerDownConfig)
{
                             ...
                             const ETH_MACConfigTypeDef *macconf)
{
   ...
   macregval =
       (macconf->InterPacketGapVal | macconf->SourceAddrControl |
        ...
        ((uint32_t)macconf->PortSelect << 15) | macconf->Speed |
   ...
   MODIFY_REG(heth->Instance->MACRXQC0R, ETH_MACRXQC0R_MASK,
              ETH_RECEIVEQUEUE_ENABLE_ALL);
   ...
   macregval = (ETH_MTLTXQ0OMR_TQS_0 | ETH_MTLTXQ0OMR_TQS_1 |
                ETH_MTLTXQ0OMR_TQS_2 | ETH_TRANSMITQUEUE_ENABLE_ALL);

   /* Write to MTLTXQ0OMR */
   MODIFY_REG(heth->Instance->MTLTXQ0OMR, ETH_MTLTXQ0OMR_MASK,
              macregval | macconf->TransmitQueueMode);
   ...
   macregval =
       (macconf->ReceiveQueueMode |
        ...
        ETH_MTLRXQ0OMR_RQS_0 | ETH_MTLRXQ0OMR_RQS_1 | ETH_MTLRXQ0OMR_RQS_2);
                             ...
                             const ETH_DMAConfigTypeDef *dmaconf)
{
   ...
   dmaregval =
       (((uint32_t)dmaconf->AddressAlignedBeats << 12) | dmaconf->BurstMode |
        dmaconf->RxOSRLimit | dmaconf->TxOSRLimit | dmaconf->AXIBLENMaxSize);
...
static void ETH_MACDMAConfig(ETH_HandleTypeDef *heth)
{
   ...
   macDefaultConf.TransmitQueueMode =
       ETH_TRANSMITSTOREFORWARD | ETH_TRANSMITQUEUE_ENABLE_ALL;
                           ...
                           uint32_t ItMode)
{
   ...
   if (READ_BIT(pTxConfig->Attributes, ETH_TX_PACKETS_FEATURES_VLANTAG) !=
      ...
      if (READ_BIT(pTxConfig->Attributes,
                   ETH_TX_PACKETS_FEATURES_INNERVLANTAG) != (uint32_t)RESET) {
