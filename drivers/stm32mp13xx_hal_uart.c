                          ...
                          pUART_CallbackTypeDef pCallback)
{
   ...
   if (huart->gState == HAL_UART_STATE_READY) {
      switch (CallbackID) {
         ...
         case HAL_UART_ERROR_CB_ID: huart->ErrorCallback = pCallback; break;
         ...
         case HAL_UART_WAKEUP_CB_ID: huart->WakeupCallback = pCallback; break;
         ...
         case HAL_UART_MSPINIT_CB_ID: huart->MspInitCallback = pCallback; break;
                            ...
                            HAL_UART_CallbackIDTypeDef CallbackID)
{
   ...
   if (HAL_UART_STATE_READY == huart->gState) {
      switch (CallbackID) {
         ...
         case HAL_UART_MSPINIT_CB_ID: break;

         case HAL_UART_MSPDEINIT_CB_ID: break;
...
#ifdef HAL_MDMA_MODULE_ENABLED
      /* Abort the UART MDMA Tx channel : use blocking DMA Abort API (no
       * callback) */
      if (huart->hmdmatx != NULL) {
...
#ifdef HAL_MDMA_MODULE_ENABLED
      /* Abort the UART MDMA Rx channel : use blocking DMA Abort API (no
       * callback) */
      if (huart->hmdmarx != NULL) {
         /* Set the UART MDMA Abort callback to Null.
            No call back execution at end of DMA abort procedure */
...
#ifdef HAL_MDMA_MODULE_ENABLED
      /* Abort the UART MDMA Tx channel : use blocking MDMA Abort API (no
       * callback) */
      if (huart->hmdmatx != NULL) {
         /* Set the UART MDMA Abort callback to Null.
            No call back execution at end of MDMA abort procedure */
...
#ifdef HAL_MDMA_MODULE_ENABLED
      /* Abort the UART MDMA Rx channel : use blocking MDMA Abort API (no
       * callback) */
      if (huart->hmdmarx != NULL) {
         /* Set the UART MDMA Abort callback to Null.
            No call back execution at end of MDMA abort procedure */
...
#ifdef HAL_MDMA_MODULE_ENABLED
      /* Abort the UART MDMA Tx channel : use non blocking MDMA Abort API
       * (callback) */
      if (huart->hmdmatx != NULL) {
...
#ifdef HAL_MDMA_MODULE_ENABLED
      /* Abort the UART MDMA Rx channel : use non blocking MDMA Abort API
       * (callback) */
      if (huart->hmdmarx != NULL) {
         ...
         */
         ...
         if ((HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR)) ||
            ...
            if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR)) {
               ...
               if (huart->hdmarx != NULL) {
                  /* Set the UART DMA Abort callback :
                     will lead to call HAL_UART_ErrorCallback() at end of DMA
                     abort procedure */
                  huart->hdmarx->XferAbortCallback = UART_DMAAbortOnError;

                  /* Abort DMA RX */
                  if (HAL_DMA_Abort_IT(huart->hdmarx) != HAL_OK) {
                     /* Call Directly huart->hdmarx->XferAbortCallback function
                      * in case of error */
                     huart->hdmarx->XferAbortCallback(huart->hdmarx);
                  }
               }
               ...
               /* Abort the UART MDMA Rx channel */
                  ...
                  /* Set the UART MDMA Abort callback :
                      will lead to call HAL_UART_ErrorCallback() at end of MDMA
                     abort procedure */
                  huart->hmdmarx->XferAbortCallback = UART_MDMAAbortOnError;

                  /* Abort MDMA RX */
                  if (HAL_MDMA_Abort_IT(huart->hmdmarx) != HAL_OK) {
                     /* Call Directly huart->hmdmarx->XferAbortCallback function
                      * in case of error */
                     huart->hmdmarx->XferAbortCallback(huart->hmdmarx);
                  }
               }
   ...
   if ((huart->ReceptionType == HAL_UART_RECEPTION_TOIDLE) &&
      ...
      if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR)) {
         ...
         /* Check received length : If all expected data are received, do
            nothing, (DMA cplt callback will be called). Otherwise, if at least
            one data has already been received, IDLE event is to be notified to
            user */
         ...
         /* Check received length : If all expected data are received, do
            nothing. Otherwise, if at least one data has already been received,
            IDLE event is to be notified to user */
...
HAL_StatusTypeDef UART_SetConfig(UART_HandleTypeDef *huart)
{
   ...
   if (huart->Init.OverSampling == UART_OVERSAMPLING_8) {
      switch (clocksource) {
         case UART_CLOCKSOURCE_PCLK1: pclk = HAL_RCC_GetPCLK1Freq(); break;
         case UART_CLOCKSOURCE_PCLK2: pclk = HAL_RCC_GetPCLK2Freq(); break;
         case UART_CLOCKSOURCE_PCLK5: pclk = HAL_RCC_GetPCLK5Freq(); break;
         case UART_CLOCKSOURCE_PCLK6: pclk = HAL_RCC_GetPCLK6Freq(); break;
         ...
         case UART_CLOCKSOURCE_PLL4Q:
            ...
            break;
         case UART_CLOCKSOURCE_HSI: pclk = (uint32_t)HSI_VALUE; break;
         case UART_CLOCKSOURCE_CSI: pclk = (uint32_t)CSI_VALUE; break;
         case UART_CLOCKSOURCE_HSE: pclk = (uint32_t)HSE_VALUE; break;
         default:
            pclk = 0U;
            ret  = HAL_ERROR;
            break;
...
/**
 * @brief  MDMA UART communication abort callback, when initiated by HAL
 * services on Error (To be called at end of MDMA Abort procedure following
 * error occurrence).
 * @param  hmdma MDMA handle.
 * @retval None
 */
 ...
 *         (To be called at end of MDMA Tx Abort procedure following user abort
 * request).
 * @note   When this callback is executed, User Abort complete call back is
 * called only if no Abort still ongoing for Rx MDMA Handle.
 * @param  hmdma MDMA handle.
 * @retval None
 */
static void UART_MDMATxAbortCallback(MDMA_HandleTypeDef *hmdma)
{
   ...
   /* No Abort process still ongoing : All MDMA channels are aborted, call user
    * Abort Complete callback */
   huart->TxXferCount = 0U;
   huart->RxXferCount = 0U;
 ...
 *         (To be called at end of MDMA Rx Abort procedure following user abort
 * request).
 * @note   When this callback is executed, User Abort complete call back is
 * called only if no Abort still ongoing for Tx MDMA Handle.
 * @param  hmdma MDMA handle.
 * @retval None
 */
static void UART_MDMARxAbortCallback(MDMA_HandleTypeDef *hmdma)
...
/**
 * @brief  MDMA UART Tx communication abort callback, when initiated by user by
 * a call to HAL_UART_AbortTransmit_IT API (Abort only Tx transfer) (This
 * callback is executed at end of MDMA Tx Abort procedure following user abort
...
/**
 * @brief  MDMA UART Rx communication abort callback, when initiated by user by
 * a call to HAL_UART_AbortReceive_IT API (Abort only Rx transfer) (This
 * callback is executed at end of MDMA Rx Abort procedure following user abort
...
{
   UART_HandleTypeDef *huart =
       (UART_HandleTypeDef *)((MDMA_HandleTypeDef *)hmdma)->Parent;
